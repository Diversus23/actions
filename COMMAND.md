...
- [actions](#) Программная среда CI/CD для платформы 1С и не только. Позволяет выполнять сборку, доставку, тестирование ПО и многое из того, что может понадобиться в DevOps
  ...
  - [fs](#fs) Команды для работы с файловой системой
    ...
    - [copy](#fs-copy) Копирует файлы и каталоги, включая подкаталоги
...
Синонимы: [--project, -p].
- *parts*: Из скольки частей состоит версия. Пример: "1.0.0.0" - 4, "1.0.0" - 3. По умолчанию 4 (не обязательный).
...
- *path*: Указывает расположение и имена файлов, которые требуется скопировать (обязательный).
Синонимы: [--path, --src, --from, -f].
- *destination*: Расположение или имена новых файлов (обязательный).
Синонимы: [--destination, --dst, --to, -t].
- *move*: Выполнить перемещение файлов (удалить источник после копирования). По умочланию выключен (не обязательный).
