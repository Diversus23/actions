...
* Добавлена команда markdown convert для конвертации произвольного файла Markdown в HTML по шаблону оформления (без разбивки на версии). Конвертация выполняется библиотекой oscript-md, внешние приложения не запускаются.
* Добавлена команда markdown templates для вывода списка шаблонов, доступных команде markdown convert.
* Команда changelog convert переведена с внешнего приложения docsinum на библиотеку oscript-md. Из состава пакета удален бинарный файл `docsinum.exe` (12 Мб), который поставлялся только для Windows: формирование HTML теперь работает одинаково на Windows, Linux и macOS и не требует запуска внешних процессов.
