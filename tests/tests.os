...
Процедура Тест_Должен_ПроверитьКонтрольнуюСуммуФайла() Экспорт
	...
	ПутьФайлаТеста = ОбъединитьПути(КаталогТестовыхФикстур(), "checksum.json");
	ПутьФайлаТестаВременный = ВременныеФайлы.СоздатьФайл("json");
	...
	Ожидаем.Что(Хэш).Равно("bd1b4ef411d37593e1e2294dea8dc499");
