...
Процедура Тест_Должен_Выполнить_Команду_infobase_check() Экспорт

	ПутьИБ = СоздатьВременнуюИБ1С();
	мПараметры = СтрШаблон("--connection /F%1", ПутьИБ);
	ЯВыполняюКомандуПродуктаCПередачейПараметров("infobase check", мПараметры);
	УдалитьФайлы(ПутьИБ);
...
Функция ПолучитьСписокТестов(Тестирование) Экспорт
	...
	Если ЭтоЛокальноеТестирование Тогда
		...
		ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_infobase_check");		
