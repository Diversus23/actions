...
Процедура Тест_Должен_Выполнить_Команду_netlenka() Экспорт

	ЯВыполняюКомандуПродуктаCПередачейПараметров("netlenka", "");
	
КонецПроцедуры

Процедура Тест_Должен_Выполнить_Команду_netlenka_module() Экспорт

	ПутьФайлаТеста = ОбъединитьПути(КаталогТестовыхФикстур(), "code.os");
	ПутьФайлаТестаВременный = юТест.ИмяВременногоФайла("txt");
	мПараметры = "--in """ + ПутьФайлаТеста + """"
		+ " --out """ + ПутьФайлаТестаВременный + """";
	ЯВыполняюКомандуПродуктаCПередачейПараметров("netlenka module", мПараметры);
...
Процедура Тест_Должен_Выполнить_Команду_changelog_templates() Экспорт

	ЯВыполняюКомандуПродуктаCПередачейПараметров("changelog templates", "");
...
Процедура Тест_Должен_Выполнить_Команду_changelog_txt() Экспорт
	...
	Команда = "--in ""%1"" --out ""%2"" --template %3";
	мПараметры = СтрШаблон(Команда, ПутьФайлаТеста, ПутьФайлаТестаРезультат, "SimpleTextWithoutVersion.txt");
...
Процедура Тест_Должен_Выполнить_Команду_changelog_txt_version() Экспорт
	...
	ПутьФайлаТеста = ОбъединитьПути(КаталогТестовыхФикстур(), "changelog.md");
	ПутьФайлаТестаРезультат = юТест.ИмяВременногоФайла("txt");
	ПутьФайлаТестаСравнить = ОбъединитьПути(КаталогТестовыхФикстур(), "changelog_1.0.0.1.txt");
	Команда = "--in ""%1"" --out ""%2"" --template %3 --version %4";
	мПараметры = СтрШаблон(Команда, ПутьФайлаТеста, ПутьФайлаТестаРезультат, "SimpleTextWithoutVersion.txt", "1.0.0.1");
	ЯВыполняюКомандуПродуктаCПередачейПараметров("changelog convert", мПараметры);
...
Процедура Тест_Должен_Выполнить_Команду_changelog_htmlfull() Экспорт
	...
	Команда = "--in ""%1"" --out ""%2"" --template %3";
	мПараметры = СтрШаблон(Команда, ПутьФайлаТеста, ПутьФайлаТестаРезультат, "Standart.html");
	ЯВыполняюКомандуПродуктаCПередачейПараметров("changelog convert", мПараметры);
...
Процедура Тест_Должен_Выполнить_Команду_changelog_check_version() Экспорт

	ПутьФайлаТеста = ОбъединитьПути(КаталогТестовыхФикстур(), "changelog.md");
	Команда = "--file ""%1"" --version ""%2""";
	мПараметры = СтрШаблон(Команда, ПутьФайлаТеста, "1.0.0.1");
	ЯВыполняюКомандуПродуктаCПередачейПараметров("changelog check-version", мПараметры);
	
КонецПроцедуры

Процедура Тест_Должен_Выполнить_Команду_changelog_check_version_Ошибка() Экспорт

	ПутьФайлаТеста = ОбъединитьПути(КаталогТестовыхФикстур(), "changelog.md");
	Команда = "--file ""%1"" --version ""%2""";
	мПараметры = СтрШаблон(Команда, ПутьФайлаТеста, "1.0.0");
	ЯВыполняюКомандуПродуктаCПередачейПараметров("changelog check-version", мПараметры, 1);
...
Процедура Тест_Должен_Выполнить_Команду_changelog_html() Экспорт
	...
	ПутьФайлаТестаСравнить = ОбъединитьПути(КаталогТестовыхФикстур(), "changelog.Simple.html");
	Команда = "--in ""%1"" --out ""%2"" --template %3";
	мПараметры = СтрШаблон(Команда, ПутьФайлаТеста, ПутьФайлаТестаРезультат, "Simple.html");
	ЯВыполняюКомандуПродуктаCПередачейПараметров("changelog convert", мПараметры);
...
Процедура Тест_Должен_Выполнить_Команду_markdown() Экспорт

	ЯВыполняюКомандуПродуктаCПередачейПараметров("markdown", "");

КонецПроцедуры

Процедура Тест_Должен_Выполнить_Команду_markdown_templates() Экспорт

	ЯВыполняюКомандуПродуктаCПередачейПараметров("markdown templates", "");

КонецПроцедуры

Процедура Тест_Должен_Выполнить_Команду_markdown_convert() Экспорт

	// oscript ../src/actions.os markdown convert --in "./fixtures/readme.md" --out "./fixtures/readme.Simple.html" --template "Simple.html"
	ПутьФайлаТеста = ОбъединитьПути(КаталогТестовыхФикстур(), "readme.md");
	ПутьФайлаТестаРезультат = юТест.ИмяВременногоФайла("html");
	ПутьФайлаТестаСравнить = ОбъединитьПути(КаталогТестовыхФикстур(), "readme.Simple.html");
	Команда = "--in ""%1"" --out ""%2"" --template %3";
	мПараметры = СтрШаблон(Команда, ПутьФайлаТеста, ПутьФайлаТестаРезультат, "Simple.html");
	ЯВыполняюКомандуПродуктаCПередачейПараметров("markdown convert", мПараметры);
...
Процедура Тест_Должен_Выполнить_Команду_markdown_convert_Заголовок() Экспорт

	ПутьФайлаТеста = ОбъединитьПути(КаталогТестовыхФикстур(), "readme.md");
	ПутьФайлаТестаРезультат = юТест.ИмяВременногоФайла("html");
	Команда = "--in ""%1"" --out ""%2"" --template %3 --title ""%4""";
	мПараметры = СтрШаблон(Команда, ПутьФайлаТеста, ПутьФайлаТестаРезультат, "Standart.html", "Личный кабинет");
	ЯВыполняюКомандуПродуктаCПередачейПараметров("markdown convert", мПараметры);

	Текст = ПолучитьТекстИзФайла(ПутьФайлаТестаРезультат);

	Ожидаем.Что(СтрНайти(Текст, "<title>Личный кабинет</title>"),
		"Заголовок из опции --title должен попасть в результат").Больше(0);
	Ожидаем.Что(СтрНайти(Текст, "{{"),
		"В результате не должно остаться неразрешенных подстановок вида {{Version}}").Равно(0);

КонецПроцедуры

Процедура Тест_Должен_Выполнить_Команду_markdown_convert_Ошибка_НеHTMLШаблон() Экспорт

	ПутьФайлаТеста = ОбъединитьПути(КаталогТестовыхФикстур(), "readme.md");
	ПутьФайлаТестаРезультат = юТест.ИмяВременногоФайла("txt");
	Команда = "--in ""%1"" --out ""%2"" --template %3";
	мПараметры = СтрШаблон(Команда, ПутьФайлаТеста, ПутьФайлаТестаРезультат, "SimpleText.txt");
	ЯВыполняюКомандуПродуктаCПередачейПараметров("markdown convert", мПараметры, 1);

КонецПроцедуры

Процедура Тест_Должен_Выполнить_Команду_markdown_convert_Ошибка_НетФайла() Экспорт

	ПутьФайлаТеста = ОбъединитьПути(КаталогТестовыхФикстур(), "readme-не-существует.md");
	ПутьФайлаТестаРезультат = юТест.ИмяВременногоФайла("html");
	Команда = "--in ""%1"" --out ""%2"" --template %3";
	мПараметры = СтрШаблон(Команда, ПутьФайлаТеста, ПутьФайлаТестаРезультат, "Simple.html");
	ЯВыполняюКомандуПродуктаCПередачейПараметров("markdown convert", мПараметры, 1);
...
Процедура Тест_Должен_Выполнить_Команду_fs_read() Экспорт

	ИмяФайла = ОбъединитьПути(КаталогТестовыхФикстур(), "checksum.txt");
	мПараметры = СтрШаблон("--file ""%1""", ИмяФайла);
	ЯВыполняюКомандуПродуктаCПередачейПараметров("fs read", мПараметры);
...
Процедура Тест_Должен_Выполнить_Команду_fs_pathexists() Экспорт

	мПараметры = "--path " + КаталогТестовыхФикстур();
	ЯВыполняюКомандуПродуктаCПередачейПараметров("fs pathexists", мПараметры);
...
Процедура Тест_Должен_Выполнить_Команду_ibcmd() Экспорт

	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd", "");
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd extension", "");

КонецПроцедуры

Процедура Тест_Должен_Выполнить_Команду_ibcmd_ОшибкиПодключения() Экспорт

	// Заданы и файловая, и серверная ИБ
	мПараметры = "--dbpath x --dbms PostgreSQL --dbserver s --dbname n";
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd configcheck", мПараметры, 1);

	// Не задано подключение
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd configcheck", "", 1);

	// Не задан сервер СУБД
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd configcheck", "--dbms PostgreSQL --dbname n", 1);

	// Недопустимый тип СУБД
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd configcheck", "--dbms Oracle --dbserver s --dbname n", 1);

КонецПроцедуры

Процедура Тест_Должен_Выполнить_Команду_ibcmd_ОшибкиОпций() Экспорт

	// Интерактивные режимы в конвейере запрещены
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd configapply", "--dbpath x --dynamic prompt", 1);
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd configapply", "--dbpath x --sessionterminate prompt", 1);

	// Нужно что-то одно: --name или --all
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd extension delete", "--dbpath x --name a --all", 1);
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd extension delete", "--dbpath x", 1);

	// Не задано ни одно свойство и недопустимое значение свойства
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd extension properties", "--dbpath x --name a", 1);
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd extension properties", "--dbpath x --name a --active maybe", 1);

	// Создание ИБ нельзя совмещать со сборкой расширения
	мПараметры = СтрШаблон("--dbpath x --project ""%1"" --create --extension a", КаталогТестов());
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd build", мПараметры, 1);

КонецПроцедуры

Функция СоздатьВременнуюИБIbcmd()

	// BSLLS:MissingTemporaryFileDeletion-off
	ПутьИБ = ОбъединитьПути(КаталогВременныхФайлов(), ПолучитьИмяВременногоФайла());
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd create", СтрШаблон("--dbpath ""%1""", ПутьИБ));
	Возврат ПутьИБ;
	// BSLLS:MissingTemporaryFileDeletion-on

КонецФункции

Процедура Тест_Должен_Выполнить_Команду_ibcmd_config() Экспорт

	ПутьИБ = СоздатьВременнуюИБIbcmd();
	Подключение = СтрШаблон("--dbpath ""%1""", ПутьИБ);
	ПутьИБИзШаблона = ОбъединитьПути(КаталогВременныхФайлов(), ПолучитьИмяВременногоФайла());
	ФайлCF = ОбъединитьПути(КаталогВременныхФайлов(), ПолучитьИмяВременногоФайла("cf"));
	ФайлDT = ОбъединитьПути(КаталогВременныхФайлов(), ПолучитьИмяВременногоФайла("dt"));
	КаталогXML = ОбъединитьПути(КаталогВременныхФайлов(), ПолучитьИмяВременногоФайла());

	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd configsave",
		СтрШаблон("%1 --file ""%2""", Подключение, ФайлCF));
	Ожидаем.Что(ФайловыеОперации.ФайлСуществует(ФайлCF), "Файл конфигурации сохранен").ЭтоИстина();

	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd create",
		СтрШаблон("--dbpath ""%1"" --template ""%2""", ПутьИБИзШаблона, ФайлCF));

	// Каталог выгрузки не пустой - команда должна его очистить
	ФайловыеОперации.ОбеспечитьКаталог(КаталогXML);
	ФайловыеОперации.ЗаписатьТекстФайла(ОбъединитьПути(КаталогXML, "junk.txt"), "junk");
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd configsavetoxml",
		СтрШаблон("%1 --path ""%2""", Подключение, КаталогXML));
	Ожидаем.Что(ФайловыеОперации.ФайлСуществует(ОбъединитьПути(КаталогXML, "Configuration.xml")),
		"Конфигурация выгружена в XML").ЭтоИстина();
	Ожидаем.Что(ФайловыеОперации.ФайлСуществует(ОбъединитьПути(КаталогXML, "junk.txt")),
		"Каталог выгрузки очищен").ЭтоЛожь();

	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd configloadfromxml",
		СтрШаблон("%1 --path ""%2""", Подключение, КаталогXML));
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd configloadfromxml",
		СтрШаблон("%1 --path ""%2"" --noapply", Подключение, КаталогXML));
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd configapply", Подключение + " --dynamic disable");
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd configcheck", Подключение);
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd configload",
		СтрШаблон("%1 --file ""%2""", Подключение, ФайлCF));

	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd dump",
		СтрШаблон("%1 --file ""%2""", Подключение, ФайлDT));
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd restore",
		СтрШаблон("%1 --file ""%2""", Подключение, ФайлDT));

	УдалитьФайлы(ПутьИБ);
	УдалитьФайлы(ПутьИБИзШаблона);
	УдалитьФайлы(ФайлCF);
	УдалитьФайлы(ФайлDT);
...
Процедура Тест_Должен_Выполнить_Команду_ibcmd_extension() Экспорт

	ИмяРасширения = "ТестовоеРасширение";
	КаталогИсходников = ОбъединитьПути(КаталогТестовыхФикстур(), "ibcmd", "xml-extension");
	ПутьИБ = СоздатьВременнуюИБIbcmd();
	Подключение = СтрШаблон("--dbpath ""%1"" --name %2", ПутьИБ, ИмяРасширения);
	ФайлCFE = ОбъединитьПути(КаталогВременныхФайлов(), ПолучитьИмяВременногоФайла("cfe"));
	КаталогXML = ОбъединитьПути(КаталогВременныхФайлов(), ПолучитьИмяВременногоФайла());
	ТекстВывода = "";

	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd extension loadfromsrc",
		СтрШаблон("%1 --path ""%2""", Подключение, КаталогИсходников));
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd extension list",
		СтрШаблон("--dbpath ""%1""", ПутьИБ), 0, ТекстВывода);
	Ожидаем.Что(ТекстВывода, "Расширение загружено").Содержит(ИмяРасширения);

	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd extension properties", Подключение + " --active no");
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd extension savetosrc",
		СтрШаблон("%1 --path ""%2""", Подключение, КаталогXML));
	Ожидаем.Что(ФайловыеОперации.ФайлСуществует(ОбъединитьПути(КаталогXML, "Configuration.xml")),
		"Расширение выгружено в XML").ЭтоИстина();

	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd extension save",
		СтрШаблон("%1 --file ""%2""", Подключение, ФайлCFE));
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd extension update", Подключение);
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd extension delete", Подключение);
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd extension load",
		СтрШаблон("%1 --file ""%2""", Подключение, ФайлCFE));
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd extension delete",
		СтрШаблон("--dbpath ""%1"" --all", ПутьИБ));

	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd extension list",
		СтрШаблон("--dbpath ""%1""", ПутьИБ), 0, ТекстВывода);
	Ожидаем.Что(ТекстВывода, "Расширения удалены").Не_().Содержит(ИмяРасширения);

	УдалитьФайлы(ПутьИБ);
	УдалитьФайлы(ФайлCFE);
	УдалитьФайлы(КаталогXML);

КонецПроцедуры

Процедура Тест_Должен_Выполнить_Команду_ibcmd_СкрытиеПароля() Экспорт

	Пароль = "Се кр&ет""123";
	ПутьИБ = СоздатьВременнуюИБIbcmd();
	КаталогКонтроля = ОбъединитьПути(КаталогВременныхФайлов(), ПолучитьИмяВременногоФайла());
	мПараметры = СтрШаблон("--dbpath ""%1"" --user Администратор --password ""%2"" --processpath ""%3""",
		ПутьИБ, СтрЗаменить(Пароль, """", """"""), КаталогКонтроля);
	ТекстВывода = "";

	// В ИБ нет пользователей, поэтому авторизация завершится ошибкой - важен только вывод
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd configcheck", мПараметры, 1, ТекстВывода);

	// Тестовый запуск включает отладочные логи всех библиотек. Библиотека cli в отладочном логе
	// выводит значения всех опций, это ее поведение, поэтому ее строки не проверяем.
	Для Каждого СтрокаВывода Из СтрРазделить(ТекстВывода, Символы.ПС, Ложь) Цикл
		Если СтрНайти(СтрокаВывода, ".lib.cli") > 0 Тогда
			Продолжить;
		КонецЕсли;
		Ожидаем.Что(СтрокаВывода, "Пароль не выводится").Не_().Содержит("кр&ет");
	КонецЦикла;
	Ожидаем.Что(ТекстВывода, "Пароль замаскирован").Содержит("--password=***");

	ФайлыКонтроля = НайтиФайлы(КаталогКонтроля, "*.json");
	Ожидаем.Что(ФайлыКонтроля.Количество(), "Записан файл контроля процесса").Больше(0);
	Для Каждого ФайлКонтроля Из ФайлыКонтроля Цикл
		ТекстФайла = ФайловыеОперации.ПрочитатьТекстФайла(ФайлКонтроля.ПолноеИмя);
		Ожидаем.Что(ТекстФайла, "Пароль не записан в файл контроля").Не_().Содержит("кр&ет");
	КонецЦикла;

	УдалитьФайлы(ПутьИБ);
	УдалитьФайлы(КаталогКонтроля);

КонецПроцедуры

Процедура Тест_Должен_Выполнить_Команду_ibcmd_build() Экспорт

	ИмяРасширения = "ТестовоеРасширение";
	ПроектКонфигурации = ОбъединитьПути(КаталогТестовыхФикстур(), "ibcmd", "edt-configuration");
	ПроектРасширения = ОбъединитьПути(КаталогТестовыхФикстур(), "ibcmd", "edt-extension");
	ПутьИБ = ОбъединитьПути(КаталогВременныхФайлов(), ПолучитьИмяВременногоФайла());
	Подключение = СтрШаблон("--dbpath ""%1""", ПутьИБ);
	ТекстВывода = "";

	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd build",
		СтрШаблон("%1 --project ""%2"" --create", Подключение, ПроектКонфигурации));
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd build",
		СтрШаблон("%1 --project ""%2""", Подключение, ПроектКонфигурации));
	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd build",
		СтрШаблон("%1 --project ""%2"" --extension %3", Подключение, ПроектРасширения, ИмяРасширения));

	ЯВыполняюКомандуПродуктаCПередачейПараметров("ibcmd extension list", Подключение, 0, ТекстВывода);
	Ожидаем.Что(ТекстВывода, "Расширение собрано из проекта EDT").Содержит(ИмяРасширения);
...
Процедура Тест_Должен_Выполнить_Команду_infobase_vanessaautomation() Экспорт
	...
	мПараметры = "";
	ЯВыполняюКомандуПродуктаCПередачейПараметров("vanessa-automation", мПараметры);
...
Процедура Тест_Должен_Выполнить_Команду_allure() Экспорт

	мПараметры = "";
	ЯВыполняюКомандуПродуктаCПередачейПараметров("allure", мПараметры);
	
КонецПроцедуры

Процедура Тест_Должен_Выполнить_Команду_allure_docker_service() Экспорт

	мПараметры = "";
	ЯВыполняюКомандуПродуктаCПередачейПараметров("allure-docker-service", мПараметры);
	
КонецПроцедуры

Процедура Тест_Должен_Выполнить_Команду_pause() Экспорт

	мПараметры = "-t 1";
	ЯВыполняюКомандуПродуктаCПередачейПараметров("pause", мПараметры);
	
КонецПроцедуры

Процедура Тест_Должен_Выполнить_Команду_dbgs() Экспорт

	мПараметры = "";
	ЯВыполняюКомандуПродуктаCПередачейПараметров("dbgs", мПараметры);
	
КонецПроцедуры

Процедура Тест_Должен_Выполнить_Команду_measurement() Экспорт

	мПараметры = "";
	ЯВыполняюКомандуПродуктаCПередачейПараметров("measurement", мПараметры);
	
КонецПроцедуры

Процедура Тест_Должен_Выполнить_Команду_measurement_emailable_report() Экспорт

	ПутьФайлов = ОбъединитьПути(КаталогТестовыхФикстур(), "measurement");
	ПутьФайла = юТест.ИмяВременногоФайла("html");

	мПараметры = СтрШаблон("--path %1 --file %2", ПутьФайлов, ПутьФайла);
	ЯВыполняюКомандуПродуктаCПередачейПараметров("measurement emailable-report", мПараметры);
	ФайлСтартераСуществует = ФайловыеОперации.ФайлСуществует(ПутьФайла);
	Ожидаем.Что(ФайлСтартераСуществует,
		"Ожидаем, что отчет emailable-report существует, а его нет. " + ПутьФайла).Равно(Истина);
...
Процедура Тест_Должен_Выполнить_Команду_edt_versionconfig_epf() Экспорт

	Версия = РаботаEDT.ВерсияОтчетаОбработкиИзПроекта(КаталогТестовыхФикстур());
	Утверждения.ПроверитьИстину(Версия = "1.0.0.2", "edt_versionconfig_epf версия не совпадает");

	мПараметры = СтрШаблон("--path ""%1""", КаталогТестовыхФикстур());
	ЯВыполняюКомандуПродуктаCПередачейПараметров("edt versionepferf", мПараметры);
...
Функция ПолучитьСписокТестов(Тестирование) Экспорт
	...
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_changelog_templates");

	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_changelog_check_version");
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_changelog_check_version_Ошибка");

	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_markdown");
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_markdown_templates");
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_markdown_convert");
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_markdown_convert_Заголовок");
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_markdown_convert_Ошибка_НеHTMLШаблон");
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_markdown_convert_Ошибка_НетФайла");
	...
	// Написать тест "Тест_Должен_Выполнить_Команду_json_filevalue"
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_fs");
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_fs_read");
	...
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_fs_pathexists");
	...
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_edt_versionconfig_epf");
	...
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_ibcmd");
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_ibcmd_ОшибкиПодключения");
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_ibcmd_ОшибкиОпций");
	// Тесты ibcmd включаются по наличию утилиты, а не по имени компьютера
	Если РаботаIbcmd.ПроверитьУстановкуIbcmd() Тогда
		ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_ibcmd_config");
		ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_ibcmd_СкрытиеПароля");
		// Фикстуры tests/fixtures/ibcmd выгружены в формате платформы 8.5
		Если ЗначениеЗаполнено(Платформа1С.ПутьКIBCMD("8.5")) Тогда
			ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_ibcmd_extension");
			Если РаботаEDT.ПроверитьУстановкуEdt() Тогда
				ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_ibcmd_build");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_allure");
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_allure_docker_service");
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_pause");
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_measurement");
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_measurement_emailable_report");
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_dbgs");
	ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_netlenka");
	...
	Если ЭтоЛокальноеТестирование Тогда
		...
		ДобавитьТест(СписокТестов, "Тест_Должен_Выполнить_Команду_netlenka_module");
		...
		// Написать тест "Тест_Должен_Выполнить_Команду_dbgs_on"
		// Написать тест "Тест_Должен_Выполнить_Команду_dbgs_off"
		// Написать тест "Тест_Должен_Выполнить_Команду_vanessa-automation run"
		// Написать тест "Тест_Должен_Выполнить_Команду_vanessa-automation change-tag"
