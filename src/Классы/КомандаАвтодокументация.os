...
Процедура ВыполнитьКоманду(Знач Команда) Экспорт
	...
	АвтосборкаДокументации(Команда, СтруктураПараметров);
...
Процедура АвтосборкаДокументации(Знач Команда, Знач Параметры)
...
Процедура ПолучитьТаблицуКоманд(Команда, Таблица, Родитель, Уровень)
	...
	Попытка
		ОписаниеКоманды.ДополнениеОписания = ОбщегоНазначения.ПрочитатьДанныеИзМакета(ОписаниеКоманды.Заголовок);
	Исключение
...
Функция ОписаниеПараметра(Параметр)
	...
	Результат = "- *" + Параметр.Наименование + "*: " + Параметр.Описание + ".";
...
Функция ТекстовоеОписаниеКоманд(ВозможныеКоманды)
	...
	Для Каждого ОписаниеКоманды Из ВозможныеКоманды Цикл
		...
		Если ОписаниеКоманды.Уровень = 0 Тогда
			ИмяКоманды = ИмяКоманды + ".os";
		КонецЕсли;
		Текст = СтрШаблон("%1- [%2](#%2) %3", Отступы(ОписаниеКоманды.Уровень), ИмяКоманды, ОписаниеКоманды.Описание);
	...
	Для Каждого ОписаниеКоманды Из ВозможныеКоманды Цикл
		...
		// Аргументы команды
		ТекстПараметры = "";
		Для Каждого Параметр Из ОписаниеКоманды.Аргументы Цикл
			ТекстПараметры = ТекстПараметры	+ ОписаниеПараметра(Параметр) + ПС;
		КонецЦикла;
		Если НЕ ПустаяСтрока(ТекстПараметры) Тогда
			Результат = Результат + ПрефиксЗаголовка(ОписаниеКоманды.Уровень + 1) + " Аргументы команды" + ДС;
