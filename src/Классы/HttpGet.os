...
Процедура ВыполнитьКоманду(Знач Команда) Экспорт
	...
	Для Индекс = 1 По МаксимальноеКоличествоПараметров Цикл
		ПараметрЗаполнен = ПараметрыКоманды.Свойство(ИмяПараметраПолное(Индекс))
			И ПараметрыКоманды.Свойство(ИмяЗначениеПолное(Индекс));
	...
	Если СтрНайти(ПараметрыКоманды.Адрес, "?") > 0 Тогда
		ПолныйАдресЗапроса = ПараметрыКоманды.Адрес + "&" + ПараметрыСтрокой;
	Иначе
		ПолныйАдресЗапроса = ПараметрыКоманды.Адрес + "?" + ПараметрыСтрокой;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПараметрыКоманды["Пользователь"]) Тогда
		Аутентификация = Новый Структура();
		Аутентификация.Вставить("Пользователь", ПараметрыКоманды["Пользователь"]);
		Лог.Отладка("Baisic-аутентификация. Пользователь %1", ПараметрыКоманды["Пользователь"]);
		Аутентификация.Вставить("Пароль", ПараметрыКоманды["Пароль"]);
		Лог.Отладка("Baisic-аутентификация. Пароль %1", ПараметрыКоманды["Пароль"]);
