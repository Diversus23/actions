...
Функция ПолучитьСтруктуруПараметров(Знач Команда)
	...
	Если НЕ ФайловыеОперации.ФайлСуществует(ПараметрыКоманды.ПутьКФайлуШаблона) Тогда
		ОбщегоНазначения.СистемноеЗавершениеРаботыОшибка("Файл <%1> не существует. Параметр ""--templatehtml""",
			ПараметрыКоманды.ПутьКФайлуШаблона);
	КонецЕсли;
	ПараметрыКоманды.Вставить("Тема", ЧтениеОпций.ЗначениеОпции("subject"));
	ПараметрыКоманды.Вставить("ОтКого", ЧтениеОпций.ЗначениеОпции("from", true));
	ПараметрыКоманды.Вставить("Почта", ЧтениеОпций.ЗначениеОпции("email", true));
...
Процедура СозданиеШаблонаРассылки(Знач Параметры)
	...
	Если Ответ.КодСостояния = 200 Тогда
		...
		Если СтрНайти(Ответ, "error") <> 0 Тогда
			ОбщегоНазначения.СистемноеЗавершениеРаботыОшибка("Отправка шаблона рассылки прервана - ошибка: %1", Ответ);
		КонецЕсли;
	...
	Иначе
		...
		ОбщегоНазначения.СистемноеЗавершениеРаботыОшибка("Ошибка отправки шаблона рассылки на сервер методом POST: %1",
			Ответ.КодСостояния);
	КонецЕсли;
