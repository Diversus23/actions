...
Функция ПолучитьСтруктуруПараметров(Знач Команда)
	...
	Если НЕ ФайловыеОперации.ФайлСуществует(ПараметрыКоманды.ПутьКФайлуШаблона) Тогда
		ОбщегоНазначения.ЗавершениеРаботыОшибка("Файл <%1> не существует. Параметр ""--templatehtml""",
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыКоманды.Тема) Тогда
		ПараметрыКоманды.Тема = "Тема письма";
	КонецЕсли;
...
Процедура СозданиеШаблонаРассылки(Знач Параметры)
	...
	Таймаут					= 60;
	HTTPСоединение 			= Новый HTTPСоединение("https://api.sendsay.ru", , , , , Таймаут);
	HTTPЗапрос 				= Новый HTTPЗапрос("/general/api/v100/json/" + Параметры.Аккаунт, Заголовки);
