...
// Представление строки запуска для логов и файлов контроля (например, со скрытыми паролями)
Перем ПредставлениеСтрокиЗапуска;
...
// Устанавливает представление строки запуска, которое будет выводиться в лог
// и записываться в файлы контроля открытых приложений вместо реальной строки запуска.
// Используется, чтобы не раскрывать пароли.
//
// Параметры:
//   Представление - Строка - представление строки запуска. Пустая строка - выводить реальную строку.
//
Процедура УстановитьПредставлениеСтрокиЗапуска(Знач Представление) Экспорт
	ПредставлениеСтрокиЗапуска = Представление;
КонецПроцедуры

// Выводит стркоу запуска в лог
//
Процедура ВывестиСтрокуЗапуска() Экспорт

	СтрокаЗапуска = ПредставлениеДляЛога(СформироватьСтрокуЗапускаПроцесса());
	Лог.Информация("
		|=========
		|%1
		|=========", СтрокаЗапуска);
...
Функция ЗапуститьПроцессКоманды(ПерехватыватьПотоки = Ложь)
    ...
    Исключение
        ...
        // TODO код проверки на регулярное выражение (ниже) убрать после исправления в движке OneScript
        ОшибкиПеременныхСреды = Новый РегулярноеВыражение(
			"\(System\.ArgumentException\):[^:]+:.*HOME[^:]+:.*home");
        Если ОшибкиПеременныхСреды.Совпадает(Инфо.Описание) Тогда
			...
			ТекстОшибки = Инфо.ПодробноеОписаниеОшибки();
	...
	Процесс.Запустить();
	ЗаписатьИнформациюОЗапуске(ПредставлениеДляЛога(СтрокаЗапуска), Процесс.Идентификатор);
...
Функция СформироватьСтрокуЗапускаПроцесса()
	...
	КонецЕсли;
	Лог.Отладка("Полная строка запуска <%1>", ПредставлениеДляЛога(СтрокаЗапуска));

	Возврат СтрокаЗапуска;

КонецФункции // СформироватьСтрокуЗапускаПроцесса()

Функция ПредставлениеДляЛога(Знач СтрокаЗапуска)

	Если ЗначениеЗаполнено(ПредставлениеСтрокиЗапуска) Тогда
		Возврат ПредставлениеСтрокиЗапуска;
	КонецЕсли;
...
Процедура ПроверитьКодВозврата(Знач КодВозврата)
	Если ПроверяемКодВозврата Тогда
		Если КодВозврата < МинимальныйОжидаемыйКодВозврата ИЛИ КодВозврата > МаксимальныйОжидаемыйКодВозврата Тогда
			ТекстВывода = ПолучитьВывод();
			Если МинимальныйОжидаемыйКодВозврата <> МаксимальныйОжидаемыйКодВозврата Тогда
				ТекстОшибки = СтрШаблон(
					"Ожидали, что код возврата <%1> команды <%2> совпадет с диапазоном <%3>-<%4>, а это не так!
					|Лог команды:
					|%5",
			...
			Иначе
				ТекстОшибки = СтрШаблон(
					"Ожидали, что код возврата <%1> команды <%2> совпадет с ожидаемым <%3>, а это не так!
					|Лог команды:
					|%4",
					КодВозврата, Приложение, МинимальныйОжидаемыйКодВозврата, ТекстВывода);
			КонецЕсли;
...
Процедура ЗаписатьИнформациюОЗапуске(Знач СтрокаЗапуска, Идентификатор)
	...
	Если ЗначениеЗаполнено(КаталогКонтроляОткрытыхПриложений) Тогда		
		...
		КонецЕсли;		
		Структура = Новый Структура("id, command", Идентификатор, СтрокаЗапуска);
