...
Процедура ОписаниеКоманды(Команда) Экспорт
	...
	Команда.Опция("file f", "",
		"Путь к файлу, куда сохранить документацию. По умолчанию сохраняется в COMMAND.md (не обязательный)")
		.ТСтрока();
...
Функция ПолучитьСтруктуруПараметров(Знач Команда)
	...
	ПараметрыКоманды.Вставить("ИмяФайла", ЧтениеОпций.ЗначениеОпции("file", Ложь));
	Если НЕ ЗначениеЗаполнено(ПараметрыКоманды.ИмяФайла) Тогда
		ПараметрыКоманды.ИмяФайла = ОбъединитьПути(ОбщегоНазначения.КаталогПроекта(), "docs", "COMMAND.md");		
	КонецЕсли;
...
Функция ПривестиКMarkDown(Знач Стр)
	...
	Результат = СтрЗаменить(Результат, "Важно!", "**Важно!**");
	Возврат Результат;
...
Процедура ОбновлениеОписанияDoctor()
	
	ТекстОписания = "Выполняемые проверки:" + Символы.ПС;
	ИмяФайла = ОбъединитьПути(ОбщегоНазначения.КаталогМакетов(), "doctor.md");
	Таблица = ОбщегоНазначения.ВыполнитьПроверкуСистемы(Ложь);
	Для Каждого Проверка Из Таблица Цикл
		...
		Текст = СтрШаблон("- [x] Проверка установленного приложения **%1**. *Проблема: %2*. Ссылки для установки: %3", 
			Проверка.ИмяПрограммы, Проверка.ТекстНеУспешно, ТекстСсылки);

		ТекстОписания = ТекстОписания + Текст + Символы.ПС;
	КонецЦикла;

	ФайловыеОперации.ЗаписатьТекстФайла(ИмяФайла, ТекстОписания);
