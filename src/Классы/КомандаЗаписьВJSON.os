...
Процедура ОписаниеКоманды(Команда) Экспорт
	...
	Команда.Опция("key k", "",
		"Имя ключа в точечной нотации. Пример ""zip.add"" - добавит настройку для команды {""zip"": {""add"": ""value""}} (обязательный)")
		.ТСтрока();
	...
	Команда.Опция("file f", "",
		"Имя файла json. Если не задано, то будет дозапись в файл settings.json. Если файла нет, он будет создан (не обязательный)")
		.ТСтрока();		
...
Функция ПолучитьСтруктуруПараметров(Знач Команда)
	...
	Если НЕ ЗначениеЗаполнено(Значение) Тогда
		ОбщегоНазначения.СистемноеЗавершениеРаботыОшибка("Не задано значение ключа. Один из параметров на выбор: --string, --number, --boolean, --date");
	КонецЕсли;
