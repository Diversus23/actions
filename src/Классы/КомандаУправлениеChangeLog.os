...
Процедура ЗарегистрироватьКоманду(Знач ИмяКоманды, Знач Парсер) Экспорт
	...
	Если мЭтоИнформация Тогда
		...
		Парсер.ДобавитьИменованныйПараметрКоманды(ОписаниеКоманды,
			"-version",
			"По какой версии будет получено описание обновления. Если не задано, то будет получено по последней");
	КонецЕсли;
...
Функция ПолучитьИнформацию(Знач Параметры) Экспорт
	...
	Для Индекс = 1 По СтрЧислоСтрок(Результат) Цикл
		...
		Если СтрНачинаетсяС(Стр, "* ") Тогда
			Стр = Сред(Стр, 3);
		ИначеЕсли СтрНачинаетсяС(Стр, "- ") Тогда
			Стр = Сред(Стр, 3);
		КонецЕсли;
...
Функция МаксимальнаяВерсияИзФайлаChangeLog(ИмяФайла)
	...
	Пока Индекс < Массив.Количество() Цикл
		...
		НоваяСтрока.Версия = Элемент.Значение;
		НоваяСтрока.ВесВерсии = ОбщегоНазначения.ВесВерсии(Элемент.Значение);
		Индекс = Индекс + 1;
	КонецЦикла;
