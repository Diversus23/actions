...
Процедура ВыполнитьКоманду(Знач Команда) Экспорт
	...
	Если ПараметрыСистемы.ЭтоWindows Тогда
		...
		ТекстКоманды = СтрШаблон(
			...
			|@{id=$Process.id;command=$ProcessInfo.FileName.ToString() + ' ' + $ProcessInfo.Arguments.ToString()} | ConvertTo-Json | Out-File ""%4"";
			|", ПутьКПриложению, АргументыЗапуска, ИмяВременногоФайла, Параметры.ИмяФайла);
