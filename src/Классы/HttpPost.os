...
Процедура ОписаниеКоманды(Команда) Экспорт
	...
	Команда.Опция("jsonbody", "", 
		"Тело отправляемое в качестве json: {""param1"":""value1"", ""param2"":""value2""}"" (не обязательный)")
		.ТСтрока();		
...
Процедура ВыполнитьКоманду(Знач Команда) Экспорт
	...
	Лог.Отладка("Выполняю post-запрос к странице %1", ПараметрыКоманды.Адрес);
	...
	Если ЗначениеЗаполнено(Пользователь) Тогда
		...
		ДопПараметры = Новый Соответствие("Аутентификация", Аутентификация);
		Лог.Отладка("Baisic-аутентификация. Пользователь %1 Пароль ***", Пользователь);
	КонецЕсли;
	
	Ответ = КоннекторHTTP.Post(ПараметрыКоманды.Адрес, ПараметрыЗапроса, ТелоJSON(ПараметрыКоманды.ТелоJSON),
		Новый Структура("Заголовки", ДопПараметры));
...
// Готовит тело запроса, переданное опцией --jsonbody
//
// Опция принимает текст JSON, а коннектор ожидает уже разобранное значение и сериализует его сам.
// Без разбора строка уходила на сервер как строка JSON: вместо {"login":"..."} отправлялось
// "{\"login\":\"...\"}", и такое тело не превращается в объект на принимающей стороне.
//
// Параметры:
//  Значение    - Строка     - текст JSON из опции --jsonbody
//
// Возвращаемое значение:
//	Произвольный - разобранное значение JSON. Если текст не является JSON, возвращается как есть
//
Функция ТелоJSON(Знач Значение)
	...
	Попытка
		Возврат РаботаJSON.JSONВСтруктуру(Значение);
	Исключение
		ПараметрыСистемы.Лог().Предупреждение(
			"Опция --jsonbody не содержит корректный JSON, тело отправлено без разбора");
		Возврат Значение;
	КонецПопытки;
...
Функция ПолучитьСтруктуруПараметров(Знач Команда)
	...
	ПараметрыКоманды.Вставить("ТелоJSON", ЧтениеОпций.ЗначениеОпции("jsonbody"));
