...
Процедура ВыполнитьКоманду(Знач Команда) Экспорт
	...
	Пока Истина Цикл
		...
		Результат = НовыйПроцесс.ЗапуститьИПодождать(СтрокаЗапуска, Ложь);
		Идентификатор = Результат.Процесс.Идентификатор;
		...
		ТекстОстановки = НовыйПроцесс.Процесс.ПотокВывода.Прочитать();
		Если СтрНайти(ТекстОстановки, "stopped") = 0 Тогда
			Прервать;
		КонецЕсли;
		НовыйПроцесс.Процесс.Завершить();
