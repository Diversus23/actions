...
Процедура ВыполнитьКоманду(Знач Команда) Экспорт
	...
	Пока Истина Цикл
		...
		Команда.УстановитьПараметры(Массив);
		//Результат = Команда.ЗапуститьИПодождать(СтрокаЗапуска, Ложь);
		//Команда.УстановитьСтрокуЗапуска(КоманднаяСтрока);
		Результат = Команда.ЗапуститьПроцесс();
		Идентификатор = Результат.Идентификатор;
		Лог.Информация("Стартовал процесс dbgs PID:%1", Идентификатор);
		ТаймаутПоУмолчанию = 3000;
		Приостановить(ТаймаутПоУмолчанию);
		ТекстОстановки = Результат.ПотокВывода.Прочитать();
