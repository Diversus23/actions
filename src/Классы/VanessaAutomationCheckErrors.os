...
Процедура ПроверитьЛогФайл(Знач ИмяФайла, МассивОшибок)
	...
	Для Индекс = 1 По ТекстовыйДокумент.КоличествоСтрок() Цикл
		...
		Иначе
			// Идем пока не дойдем до начало следующего сценария или чего-то другого (накапливаем содержание ошибки)
			Если НЕ ПустаяСтрока(СтрокаСОшибкой) Тогда
				Если СтрокаОкончаниеОшибки(Стр) Тогда
...
Функция СтрокаОкончаниеОшибки(Знач Стр)

	СокращеннаяСтр = СокрЛП(Стр);
	Возврат СтрНачинаетсяС(СокращеннаяСтр, "Фича:")
		ИЛИ СтрНачинаетсяС(СокращеннаяСтр, "Сценарий:")
		ИЛИ СтрНачинаетсяС(СокращеннаяСтр, "Scenario:")
		ИЛИ СтрНачинаетсяС(СокращеннаяСтр, "Feature:") 
		ИЛИ СтрНачинаетсяС(СокращеннаяСтр, "Script:")
		ИЛИ СтрНачинаетсяС(СокращеннаяСтр, "CheckConfig:");
