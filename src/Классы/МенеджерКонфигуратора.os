...
Процедура ЗапуститьПроцесс1С(Знач СтрокаЗапуска, Знач ПутьКФайлуЛога)
	...
	Пока НЕ Процесс.Завершен Цикл
		...
		Если НадоЧитатьЛог Тогда
			ВывестиНовыеСообщения(ПутьКФайлуЛога, КолСтрокЛогаПрочитано, ЛогЗавершен);
			Если ЛогЗавершен Тогда
				Приостановить(10000);
				ОбщегоНазначения.ЗавершитьПроцессПоИдентификатору(Процесс.Идентификатор);
			КонецЕсли;
		КонецЕсли;
...
Процедура ВывестиНовыеСообщения(ИмяФайлаЛога, КолСтрокЛогаПрочитано, ЛогЗавершен)
