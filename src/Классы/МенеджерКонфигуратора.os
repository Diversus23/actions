...
Процедура ОбновитьКонфигурациюИзФайла(Знач Параметры) Экспорт
	...
	Попытка
		...
		Лог.Информация("Обновление конфигурации базы данных", Параметры.ПутьКФайлу);
...
//		* ТекстОшибки - Строка - если в процессе была ошибка, будет передано в этом свойстве
//		
Функция ПолучитьИнформациюОМетаданных(Знач Версия = Неопределено, Знач ИмяФайлаИзменений = "CHANGELOG.md") Экспорт
	...
	// Информация о макете
	Параметры = РаботаСChangeLog.КонструкторКонвертирования();
	ОписаниеИзмененийHTML = ВременныеФайлы.НовоеИмяФайла("html");	
	Параметры.ИсходныйФайл = ИмяФайлаИзменений;
	Параметры.Версия = ИнформацияОМетаданных.Версия;
	Параметры.ИмяПродукта = ИнформацияОМетаданных.Синоним;
	Параметры.ФайлРезультат = ОписаниеИзмененийHTML;
	Параметры.Шаблон = "SimpleWithoutVersion.html";
	...
	Параметры.ФайлРезультат = ОписаниеИзмененийТекст;
	Параметры.Шаблон = "SimpleTextWithoutVersion.txt";
	РаботаСChangeLog.Конвертировать(Параметры);
...
Процедура СохранитьИнформациюОбОбновлении(Знач Параметры) Экспорт	
	...
	ИнформацияОМетаданных = ПолучитьИнформациюОМетаданных(Параметры.Версия, Параметры.ИмяФайлаИзменений);
...
Процедура СоздатьОписаниеОбновленияДляРассылки(ИмяФайла, ПараметрыОбновления, JSON)
	...
	КодПродукта = "it";
	Параметры.Вставить("news_url", "https://softonit.ru/blog/news/updateit/upd" + КодПродукта + "-" + ВерсияДляНовости + "/");
	...
	//ОбщегоНазначения.ВывестиПеременнуюВКонсоль(Соответствие);
	//ОбщегоНазначения.
