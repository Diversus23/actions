...
//	Попытка
	...
	// 	Лог.Предупреждение("Причина ошибки:
	// 		|%1
...
Функция ЗапуститьВРежимеПредприятияСЛогФайлом(Знач ПараметрЗапуска,
	...
	Попытка
		...
		ОжиданиеЗавершенияТестирования1С(Процесс, ПутьДамп);
	...
	Пока НЕ Процесс.Завершен ИЛИ Процесс.ПотокВывода.ЕстьДанные ИЛИ Процесс.ПотокОшибок.ЕстьДанные Цикл
		...
		Попытка			
			...
			Если Процесс.ПотокВывода.ЕстьДанные Тогда
				ТекстИнформация = Процесс.ПотокВывода.Прочитать();
				Если НЕ НадоЧитатьЛог Тогда
					Лог.Информация(ТекстИнформация);
				КонецЕсли;
			КонецЕсли;
			ТекстОшибка = "";
			Если Процесс.ПотокОшибок.ЕстьДанные Тогда
				ТекстОшибка = Процесс.ПотокОшибок.Прочитать();
				Если НЕ НадоЧитатьЛог Тогда
					Лог.Ошибка(ТекстОшибка);
				КонецЕсли;
			КонецЕсли;
		Исключение
		КонецПопытки;
