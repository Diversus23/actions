...
Процедура ЗапуститьВРежимеПредприятияСПроверкойВыполнения(
	...
	Исключение
		...
		ОбщегоНазначения.ЗавершениеРаботыОшибка("Причина ошибки:
			|%1
	...
	Если ЗначениеЗаполнено(ПутьКФайлуСтатусаВыполнения) Тогда		
		Если ФайловыеОперации.ФайлСуществует(ПутьКФайлуСтатусаВыполнения) Тогда
			...
			ИначеЕсли СтатусВозврата = "1" Тогда
				ОбщегоНазначения.ЗавершениеРаботыОшибка("Vanessa Automation или 1С:Предприятие вернуло код возврата 1.
					|%1",
...
Функция ЗапуститьВРежимеПредприятияСЛогФайлом(Знач ПараметрЗапуска,
	...
	Исключение
		...
		ОбщегоНазначения.ЗавершениеРаботыОшибка(ОписаниеОшибки);
	КонецПопытки;
...
Процедура ЗапуститьПроцесс1С(Знач СтрокаЗапуска, Знач ПутьКФайлуЛога)
	...
	Пока НЕ Процесс.Завершен Цикл
		...
		Если НадоЧитатьЛог Тогда
			ВывестиНовыеСообщения(ПутьКФайлуЛога, КолСтрокЛогаПрочитано, Идентификатор);
		КонецЕсли;
...
Процедура ВывестиНовыеСообщения(ИмяФайлаЛога, КолСтрокЛогаПрочитано, Идентификатор)
	...
	Попытка
		...
		Для Каждого Стр Из МассивСтрок Цикл
			...
			Если СтрокаЛога = "Закрытие клиента тестирования." Тогда
				ТаймаутПоУмолчанию = 10000;
				Приостановить(ТаймаутПоУмолчанию);
				ОбщегоНазначения.ЗавершитьПроцессПоИдентификатору(Идентификатор);
			КонецЕсли;
		КонецЦикла;
	Исключение
