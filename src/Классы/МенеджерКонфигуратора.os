...
Функция ЗапуститьВРежимеПредприятияСЛогФайлом(Знач ПараметрЗапуска,
	...
	// 	Приложение = "sh -c '" + Приложение + " "+ СтрокаЗапуска + "'";
	// Иначе
	// 	Приложение = Приложение + " " + СтрокаЗапуска;
	// КонецЕсли;
	...
	Попытка
		...
		//ЗапуститьПриложение(Приложение, , Истина);
...
Процедура ЗапуститьПроцесс1С(Знач СтрокаЗапуска, Знач ПутьКФайлуЛога)
	...
	Пока НЕ Процесс.Завершен Цикл
		...
		Если НадоЧитатьЛог Тогда
			ВывестиНовыеСообщения(ПутьКФайлуЛога, КолСтрокЛогаПрочитано, ЗавершитьПоЛогу);
			Если ЗавершитьПоЛогу Тогда
				Лог.Информация("Необходимо самостоятельно завершить работу приложения");
			КонецЕсли;
		КонецЕсли;
...
Процедура ВывестиНовыеСообщения(ИмяФайлаЛога, КолСтрокЛогаПрочитано, ЗавершитьПоЛогу)
	...
	Попытка
		...
		Для Каждого Стр Из МассивСтрок Цикл
			...
			Если СтрНайти(СтрокаЛога, "Закрытие клиента тестирования") > 0
				ИЛИ СтрНайти(СтрокаЛога, "Закрытие TestClient <Этот клиент>") > 0 Тогда
				ЗавершитьПоЛогу = Истина;
			КонецЕсли;
		КонецЦикла;
	Исключение
		Лог.Ошибка(ОписаниеОшибки());
