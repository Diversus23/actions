...
//
// Параметры:
//  Параметры - Структура - настройки обновления:
//		* ПутьКФайлу - Строка - путь к cf/cfe файлу
//
Процедура ОбновитьКонфигурациюИзФайла(Знач Параметры) Экспорт
	
	Лог.Информация("Начало обновления конфигурации 1С на поддержке");
	Попытка
		Лог.Информация("Обновление конфигурации из файла <%1>", Параметры.ПутьКФайлу);
		Конфигуратор.ОбновитьКонфигурациюИзФайла(Параметры.ПутьКФайлу);
	...
	Исключение
		...
		КонецЕсли;
		ОбщегоНазначения.ЗавершениеРаботыОшибка("Произошла ошибка при обновлении ИБ: %1", ТекстОшибки);
	КонецПопытки;
...
//	ИмяФайлаИзменений - Строка - имя макета для получения описания изменений
//
// Возвращаемое значение:
...
//		* ТекстОшибки - Строка - если в процессе была ошибка, будет передано в этом свойстве
//		
Функция ПолучитьИнформациюОМетаданных(Знач Версия = Неопределено, Знач Параметры) Экспорт
	...
	// Информация о макете
	ПараметрыСборки = РаботаСChangeLog.КонструкторКонвертирования();
	ОписаниеИзмененийHTML = ВременныеФайлы.НовоеИмяФайла("html");	
	ПараметрыСборки.ИсходныйФайл = Параметры.ИмяФайлаИзменений;
	ПараметрыСборки.Версия = ИнформацияОМетаданных.Версия;
	ПараметрыСборки.ИмяПродукта = ИнформацияОМетаданных.Синоним;
	ПараметрыСборки.ФайлРезультат = ОписаниеИзмененийHTML;
	ПараметрыСборки.Шаблон = "SimpleWithoutVersion.html";
	ЗаполнитьЗначенияСвойств(ПараметрыСборки, Параметры,
		"ПутьККонфигурацииПарсера, МаксимальнаяШиринаИзображения");
	РаботаСChangeLog.Конвертировать(ПараметрыСборки);
	ИнформацияОМетаданных.Вставить("ОписаниеИзмененийHTML", ОбщегоНазначения.ПрочитатьФайл(ОписаниеИзмененийHTML));

	ОписаниеИзмененийТекст = ВременныеФайлы.НовоеИмяФайла("txt");
	ПараметрыСборки.ФайлРезультат = ОписаниеИзмененийТекст;
	ПараметрыСборки.Шаблон = "SimpleTextWithoutVersion.txt";
	РаботаСChangeLog.Конвертировать(ПараметрыСборки);
	ИнформацияОМетаданных.Вставить("ОписаниеИзмененийТекст", ОбщегоНазначения.ПрочитатьФайл(ОписаниеИзмененийТекст));
...
Процедура ВыполнитьТестированиеVanessaAutomation(Знач ПутьКФичам = Неопределено,
	...
	Знач ДопКлючи = "", 
	Знач КаталогКонтроляОткрытыхПриложений = Неопределено) Экспорт
	...
	Если НЕ ФайловыеОперации.КаталогСуществует(ПутьКФичам) И НЕ ФайловыеОперации.ФайлСуществует(ПутьКФичам) Тогда		
		Лог.Предупреждение("Фичи файлы не найдены <%1>", ПутьКФичам);
		ЗавершитьРаботу(1);
	КонецЕсли;
	...
	ЗапуститьВРежимеПредприятияСПроверкойВыполнения(
		...
		ПутьЛогаВыполненияСценариев, ПутьКФайлуСтатусаВыполнения, КаталогКонтроляОткрытыхПриложений);
...
Процедура ЗапуститьВРежимеПредприятияСПроверкойВыполнения(
				...
				Знач ПутьКФайлуСтатусаВыполнения = Неопределено,
				Знач КаталогКонтроляОткрытыхПриложений = Неопределено) Экспорт
	...
	Если ОжидатьЗавершения Тогда
		ЛогОт1С = ЗапуститьВРежимеПредприятияСЛогФайлом(
			...
			ТолстыйКлиент, ДополнительныеКлючиЗапуска, ДопСообщения, КаталогКонтроляОткрытыхПриложений);
	Иначе
		ЗапуститьВРежимеПредприятия(
			ПараметрЗапуска, ОбработкаДляЗапуска,
			ТолстыйКлиент, ДополнительныеКлючиЗапуска, ОжидатьЗавершения, ДопСообщения);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПутьКФайлуСтатусаВыполнения) Тогда		
		Если ФайловыеОперации.ФайлСуществует(ПутьКФайлуСтатусаВыполнения) Тогда
			...
			ИначеЕсли СтатусВозврата = "1" Тогда
				Лог.Предупреждение("Vanessa automation или 1С:Предприятие вернуло код возврата 1
					|" + ДопСообщения.СообщениеВСлучаеПадения);
			...
			Иначе
				...
				Лог.Предупреждение("Получен неожиданный/неверный результат работы - %1
...
Процедура СохранитьИнформациюОбОбновлении(Знач Параметры) Экспорт	
	...
	УсловиеПроверки = ЗначениеЗаполнено(Параметры.HTML) 
		ИЛИ ЗначениеЗаполнено(Параметры.TXT) 
		ИЛИ ЗначениеЗаполнено(Параметры.LETTER);
	Ожидаем.Что(УсловиеПроверки, "HTML или TXT должны быть заполнены").ЕстьИстина();

	ИнформацияОМетаданных = ПолучитьИнформациюОМетаданных(Параметры.Версия, Параметры);
	...
	Если ЗначениеЗаполнено(Параметры.LETTER) Тогда
		
		СоздатьОписаниеОбновленияДляРассылки(Параметры.LETTER, Параметры, 
			ИнформацияОМетаданных.ОписаниеИзмененийJSON);
		Лог.Информация("Записали информацию в JSON-файл <%1>", Параметры.LETTER);
...
Процедура СоздатьДистрибутив(Знач Параметры) Экспорт
	...
	Попытка		
		...
		// Делаем архив
		ИмяАрхива = ОбъединитьПути(КаталогВерсииПолный, Префикс + "_" + ВерсияСПодчеркиваниями + "_full.zip");
	...
	Если МассивВерсий.Количество() > 0 Тогда
		...
		Попытка
			...
			// Делаем архив
			ИмяАрхива = ОбъединитьПути(КаталогВерсииОбновление, Префикс + "_" + ВерсияСПодчеркиваниями + "_upd.zip");
			ЗаписьZIP =  Новый ЗаписьZipФайла(ИмяАрхива);     
...
Процедура ЗапуститьВРежимеПредприятия(Знач ПараметрЗапуска,
	...
	Исключение
		...
		Лог.Предупреждение("Ошибка запуска в режиме предприятия: %1
...
Функция ЗапуститьВРежимеПредприятияСЛогФайлом(Знач ПараметрЗапуска,
	...
	Знач ДополнительныеКлючиЗапуска,
	Знач ДопСообщения,
	Знач КаталогКонтроляОткрытыхПриложений)
	...
	ПутьДамп = ОбъединитьПути(КаталогВременныхФайлов(), Строка(Новый УникальныйИдентификатор()) + ".log");
	...
	Попытка
		...
		Команда.УстановитьКаталогКонтроляОткрытыхПриложений(КаталогКонтроляОткрытыхПриложений);
		Команда.УстановитьСтрокуЗапуска(Приложение);
		Команда.УстановитьИсполнениеЧерезКомандыСистемы(Ложь);
		Команда.ПерехватыватьПотоки(Ложь);
		Команда.ВывестиСтрокуЗапуска();
		Команда.ПоказыватьВыводНемедленно(Ложь);
		Процесс = Команда.ЗапуститьПроцесс();
		Лог.Информация("Старт ожидания");
		ОжиданиеЗавершенияТестирования1С(Процесс, ПутьЛогаВыполненияСценариев);
		Лог.Информация("Конец ожидания");
		Результат = ПоказатьЛогПредприятия(ПутьДамп, ДопСообщения.ПоказыватьДополнительноЛогПредприятия);
		Лог.Информация("Результат: %1", Результат);
	Исключение
		ОписаниеОшибки = ОписаниеОшибки();
		Результат = ПоказатьЛогПредприятия(ПутьДамп, Истина);
		Лог.Предупреждение("Ошибка запуска процесса для тестирования: %1
...
Процедура ОжиданиеЗавершенияТестирования1С(Процесс, Знач ПутьКФайлуЛога)

	ПериодОпросаВМиллисекундах = 1000;

	НадоЧитатьЛог = Истина;
	КолСтрокЛогаПрочитано = 0;
	Лог.Информация("Имя лог файла: %1", ПутьКФайлуЛога);
...
Функция ПолучитьНовыеСтрокиЛога(Знач ИмяФайла, КолСтрокЛогаПрочитано)
	...
	// Пытаемся прочитать файл с логом
	Попытка
		...
		Текст.Закрыть();			
	Исключение
		// Возможно файл заблокирован, в другой раз перечитаем
...
Процедура СоздатьФайлМанифеста(Знач ИмяФайла, Знач ИнформацияОМетаданных)
	...
	Если ЗначениеЗаполнено(КаталогОбщихФайлов) Тогда
		...
		КаталогОбщихФайлов = ФайловыеОперации.ЗаменитьРазделителиПутиНаРазделителиОС(КаталогОбщихФайлов);
	...
	Файл.ДобавитьСтроку("{0},00000000-0000-0000-0000-000000000000,"""",00000000-0000-0000-0000-000000000000,"""",""Обновление"",2,""en"",a73646ed-6000-4541-b5d7-3b76985639f8,""ru"",a73646ed-6000-4541-b5d7-3b76985639f8,");
...
Процедура СоздатьОписаниеОбновленияДляРассылки(ИмяФайла, ПараметрыОбновления, JSON)
	
	Соответствие = РаботаJSON.JSONВСтруктуру(JSON, Истина);
	...
	ВерсияДляНовости = СтрЗаменить(Версия, ".", "-");
	КодПродукта = "it";
	Параметры.Вставить("news_url", "https://softonit.ru/blog/news/update" 
		+ КодПродукта + "/upd" + КодПродукта + "-" + ВерсияДляНовости + "/");
	Параметры.Вставить("version", ПараметрыОбновления.Версия);
	
	// ToDo
	//Параметры.Вставить("news_short_description", ЧтениеОпций.ЗначениеОпции("news_short_description", Ложь));	
	//Параметры.Вставить("v8version", ЧтениеОпций.ЗначениеОпции("v8version"));
	// news_picture
	// versions_update (заменить версии на их ссылки на сайте)
	// Упорядочить разделы, как они идут в конфигураторе.
	
	Описание = "";	
	Для Каждого Раздел Из Соответствие Цикл
		ТекстШаблона = ОбщегоНазначения.ПрочитатьДанныеИзМакета("NewsLetterH2.html");
		ПараметрыРаздела = Новый Соответствие();
		ПараметрыРаздела.Вставить("text", Раздел.Ключ);
		Описание = Описание + СтроковыеОперации.ЗаменитьПараметры(ТекстШаблона, ПараметрыРаздела) + Символы.ПС;
		//
		Для Каждого Строки Из Раздел.Значение Цикл
			ПараметрыСтроки = Новый Соответствие();
			ПараметрыСтроки.Вставить("text", Строки["Текст"]);
			ТекстШаблона = ОбщегоНазначения.ПрочитатьДанныеИзМакета("NewsLetterP.html");
			Описание = Описание + СтроковыеОперации.ЗаменитьПараметры(ТекстШаблона, ПараметрыСтроки) + Символы.ПС;
		КонецЦикла;
	КонецЦикла;

	Параметры.Вставить("news_description", Описание);
	ТекстШаблона = ОбщегоНазначения.ПрочитатьДанныеИзМакета("NewsLetterTemplate.html");
	ТекстШаблона = СтроковыеОперации.ЗаменитьПараметры(ТекстШаблона, Параметры);
	ТД = Новый ТекстовыйДокумент;
	ТД.УстановитьТекст(ТекстШаблона);
	ТД.Записать(ИмяФайла);
