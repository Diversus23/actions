...
Процедура Инициализация(Знач Параметры) Экспорт
	...
	Если НЕ Работа1С.ПроверитьУстановку1C() Тогда
...
Функция ПолучитьИнформациюОМетаданных(Знач Версия = Неопределено, Знач ИмяМакета = "ОписаниеИзмененийСистемы") Экспорт
	...
	JSON = ОбщегоНазначения.СтруктураВJSON(Настройки);
	...
	ИнформацияОМетаданных = ОбщегоНазначения.JSONВСтруктуру(JSON, Ложь);
...
Процедура СоздатьДистрибутив(Знач Параметры) Экспорт
	...
	// Создаем 1cv8upd.htm
	ИмяФайла = ОбъединитьПути(КаталогВерсииДополнительныеФайлы, "1cv8upd.htm");
	СоздатьФайлДистрибутива(ИмяФайла, "DistribNewsTemplate.htm", ИнформацияОМетаданных);
