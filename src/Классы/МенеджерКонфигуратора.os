...
Процедура ВыполнитьТестированиеVanessaAutomation(Знач ПутьКФичам = Неопределено,
	...
	Если ФайловыеОперации.ФайлСуществует(ПутьКНастройкам) Тогда
		...
		Если НастройкиЗапуска.Получить("ПутьКФайлуДляВыгрузкиСтатусаВыполненияСценариев") <> Неопределено
			И НЕ ПустаяСтрока(НастройкиЗапуска.Получить("ПутьКФайлуДляВыгрузкиСтатусаВыполненияСценариев")) Тогда
			ПутьКФайлуСтатусаВыполнения = НастройкиЗапуска.Получить("ПутьКФайлуДляВыгрузкиСтатусаВыполненияСценариев");
		КонецЕсли;
		Если НастройкиЗапуска.Получить("ИмяФайлаЛогВыполненияСценариев") <> Неопределено 
			И НЕ ПустаяСтрока(НастройкиЗапуска.Получить("ИмяФайлаЛогВыполненияСценариев"))Тогда
			ПутьЛогаВыполненияСценариев = НастройкиЗапуска.Получить("ИмяФайлаЛогВыполненияСценариев");
		КонецЕсли;
	КонецЕсли;
