...
Функция ЗначениеОпции(Знач ИмяОпции, Знач ОшибкаЕслиНеЗаполнено = Ложь, Знач ОпцияКомандыРодителя = Ложь) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ЗначениеОпции) И ОшибкаЕслиНеЗаполнено = Истина Тогда
		...
		Иначе
			ПредставлениеОпции = "--" + ИмяОпции;
		КонецЕсли;
		ОбщегоНазначения.ЗавершениеРаботыОшибка("Опция ""%1"" должна быть указана", ПредставлениеОпции);
		
	КонецЕсли;

	//Сообщить(СтрШаблон("%1 = %2", ИмяОпции, ЗначениеОпции));
...
Процедура ПриСозданииОбъекта(Знач НоваяКоманда)
	...
	// ОбщегоНазначения.ВывестиМассивВКонсоль(ФайлыПараметров);
	// ОбщегоНазначения.ВывестиСтруктуруВКонсоль(ОпцииИзФайлов);
