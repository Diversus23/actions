...
Процедура ПрочитатьФайлыПараметров(ЗначенияПараметров, Знач ФайлыПараметров)
	...
	Для Каждого ТекФайл Из ФайлыПараметров Цикл
		...
		Если НЕ (ВремФайл.Существует() И ВремФайл.ЭтоФайл()) Тогда
			...
			Лог.Отладка("Файл параметров ""%1"" не найден. Продолжаем работу", ТекФайл);
			Продолжить;
		КонецЕсли;
...
Процедура ПриСозданииОбъекта(Знач НоваяКоманда)
	...
	ФайлыПараметров = Новый Массив;
	ОпцияФайлНастроек = Команда.ЗначениеОпции("settings");
	Если ЗначениеЗаполнено(ОпцияФайлНастроек) Тогда
		ФайлыПараметров.Добавить(ОпцияФайлНастроек);
	КонецЕсли; 
	ФайлНастроек1 = ОбъединитьПути(ФайловыеОперации.НайтиКаталогТекущегоПроекта("."), 
		ПараметрыСистемы.ИмяФайлаНастроек());
	ФайлыПараметров.Добавить(ФайлНастроек1);
	ФайлНастроек2 = ОбъединитьПути(ФайловыеОперации.НайтиКаталогТекущегоПроекта(".."), 
		ПараметрыСистемы.ИмяФайлаНастроек());
	ФайлыПараметров.Добавить(ФайлНастроек2);
	ФайлНастроек3 = ОбъединитьПути(ФайловыеОперации.НайтиКаталогТекущегоПроекта(".."), "..",
		ПараметрыСистемы.ИмяФайлаНастроек());
	ФайлыПараметров.Добавить(ФайлНастроек3);
	
	ПрочитатьФайлыПараметров(ОпцииИзФайлов, ФайлыПараметров);

	Для Каждого Стр Из НоваяКоманда.КомандыРодители Цикл
		Сообщить(Стр.ПолучитьИмяКоманды());
	КонецЦикла;
	Сообщить(НоваяКоманда.ПолучитьИмяКоманды());
