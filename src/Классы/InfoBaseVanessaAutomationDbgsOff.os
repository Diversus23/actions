...
Процедура ОписаниеКоманды(Команда) Экспорт
	...
	Команда.Опция("file f", "",
		"Файл-флаг для запущенного сервера отладки dbgs. По умолчанию ""dbgs.json"". После остановки сервера отладки файл будет удален (не обязательный)")
		.ТСтрока();	
...
Процедура ВыполнитьКоманду(Знач Команда) Экспорт
	...
	Если ЗначениеЗаполнено(Процесс) Тогда
		Процесс.Завершить();
		Лог.Информация("Сервер отладки остановлен на свободном порту %1 (PID %2)", Порт, Идентификатор);
