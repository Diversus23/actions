...
Процедура ВыполнитьКоманду(Знач Команда) Экспорт
	...
	РезультатЗапуска = НовыйПроцесс.ЗапуститьИПодождать(СтрокаЗапуска, Истина);

	ТаймаутПоУмолчанию = 10000;
	Приостановить(ТаймаутПоУмолчанию);
	
	Если ЗначениеЗаполнено(Параметры.ИмяФайла) И ФайловыеОперации.ФайлСуществует(Параметры.ИмяФайла) Тогда
