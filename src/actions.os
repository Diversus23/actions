...
Процедура ВыполнитьПриложение(Приложение)
    ...
    Приложение.ДобавитьКоманду("timing",
    ...
    Приложение.Опция("timingpath t", Ложь, "Профилирование работы команды (засекаем время и сохраняем в каталог)")
        .ТСтрока()
        .ВОкружении("ACTIONS_TIMINGPATH");
...
Процедура ЗафиксироватьЗамеры(Команда)
	...
	КаталогДляЗамеров = ЧтениеОпций.ЗначениеОпции("timingpath");
    Если ЗначениеЗаполнено(КаталогДляЗамеров) Тогда
        ПараметрыСистемы.Замеры.СохранитьЗамерыВКаталог(КаталогДляЗамеров);
    КонецЕсли;
    // ПараметрыСистемы.Замеры.СформироватьОтчетПоЗамерам("file.html", ".");
