...
Процедура ВыполнитьПриложение(Приложение)
    ...
    Приложение.ДобавитьКоманду("dbgs",
    ...
    Приложение.ДобавитьКоманду("epferf",
    ...
    Приложение.ДобавитьКоманду("ibcmd",
    ...
    Приложение.ДобавитьКоманду("process",
        "Работа с процессами операционной системы (ОС)",
        Новый ControlProcess());
    ...
    Приложение.Опция("measurementpath m", Ложь,
        "Каталог для сохранения оценки производительности выполнения команд (засекаем время и сохраняем в каталог)")
        .ТСтрока()
        .ВОкружении("ACTIONS_MEASUREMENTPATH");
...
Процедура ЗафиксироватьЗамеры(Команда)
    
    ПараметрыСистемы.Замеры.ЗафиксироватьВсеЗамеры();
	ЧтениеОпций = Новый ЧтениеОпцийКоманды(Команда);
	КаталогДляЗамеров = ЧтениеОпций.ЗначениеОпции("measurementpath");
    Если ЗначениеЗаполнено(КаталогДляЗамеров) И ТипЗнч(КаталогДляЗамеров) = Тип("Строка") Тогда
        ПараметрыСистемы.Замеры.СохранитьЗамерыВКаталог(КаталогДляЗамеров);
    КонецЕсли;
...
ПараметрыСистемы.Замеры = Новый МенеджерЗамеров();
...
Попытка
    ...
    ЗафиксироватьЗамеры(ГлавнаяКоманда);
