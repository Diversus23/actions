...
// Настройки парсера Markdown, см. НастройкиMarkdown()
Перем НастройкиMarkdownКэш;
...
//
// Используется набор расширений GitHub Flavored Markdown (таблицы, списки задач,
// зачеркивание, автоссылки) - он соответствует разметке, принятой в файлах ChangeLog.
//
// Параметры:
//  ТекстMD    - Строка     - исходный текст в формате Markdown
//
// Возвращаемое значение:
//	Строка - текст в формате HTML
//
Функция КонвертироватьВHTML(Знач ТекстMD) Экспорт

	Возврат Markdown.ВHTML(ТекстMD, НастройкиMarkdown());
...
Процедура Конвертировать(Знач Параметры) Экспорт
	...
	ЭтоШаблонHTML = СтрНайти(НРег(Параметры.Шаблон), ".html") > 0;
	...
	КонецПопытки;
	Попытка
		МинимизироватьHTML = Параметры.МинимизироватьHTML;
	Исключение
		МинимизироватьHTML = Ложь;
	КонецПопытки;

	ДополнитьТаблицуВерсийКолонкойТекстHTML(ТаблицаВерсий, Версия);
	...
	Для Каждого СтрокаВерсия Из ТаблицаВерсий Цикл
		...
		Если СтрЗаканчиваетсяНа(Параметры.Шаблон, ".html") Тогда
			ТекстВерсии = ОбработатьИзображенияHTML(СтрокаВерсия.ТекстHTML,
				МаксимальнаяШиринаИзображения, ИспользоватьЛенивуюЗагрузку);
		Иначе
	...
	Если ИспользоватьЛенивуюЗагрузку = Истина Тогда
		Текст = ВстроитьСкриптЛенивойЗагрузки(Текст);
	КонецЕсли;

	// Надо минимизировать HTML
	Если МинимизироватьHTML = Истина И ЭтоШаблонHTML = Истина Тогда
...
// Возвращает текст шаблона оформления по его имени
//
// Параметры:
//  ИмяШаблона    - Строка     - имя шаблона, например "Softonit.html"
//
// Возвращаемое значение:
//	Строка - текст шаблона
//
Функция ПолучитьТекстШаблона(ИмяШаблона) Экспорт
...
// Применяет к изображениям HTML-текста оформление: фиксированную ширину и ленивую загрузку
//
// Параметры:
//  Текст                         - Строка  - исходный HTML-текст
//  МаксимальнаяШиринаИзображения - Число   - если не равна 0, всем тегам img задается эта ширина
//  ИспользоватьЛенивуюЗагрузку   - Булево  - Истина - подменить src на data-src и добавить class="lazyload"
//
// Возвращаемое значение:
//	Строка - HTML-текст с обработанными изображениями
//
Функция ОбработатьИзображенияHTML(Знач Текст, Знач МаксимальнаяШиринаИзображения,
	Знач ИспользоватьЛенивуюЗагрузку) Экспорт
	...
	Если МаксимальнаяШиринаИзображения <> 0 Тогда
		...
		Результат = ОбщегоНазначения.ЗаменитьПоРегулярномуВыражению(Результат, "<img(.*)src=", СтрЗамены);
	КонецЕсли;
...
// Встраивает в HTML-документ скрипт ленивой загрузки изображений
//
// Параметры:
//  Текст    - Строка     - исходный HTML-документ
//
// Возвращаемое значение:
//	Строка - HTML-документ со встроенным скриптом. Если в документе нет ни head, ни body,
//	         текст возвращается без изменений
//
Функция ВстроитьСкриптЛенивойЗагрузки(Знач Текст) Экспорт
	...
	Возврат Лев(Текст, Индекс - 1) + ТекстСкриптаЛенивойЗагрузки() + Сред(Текст, Индекс);
...
// Функция возвращает текст скрипта ленивой загрузки
//
//	Возврат: Строка - текст скрипта
//
...
//                 <продолжение описания параметра>
//
Процедура ДополнитьТаблицуВерсийКолонкойТекстHTML(ТаблицаВерсий, Знач Версия);

	ТаблицаВерсий.Колонки.Добавить("ТекстHTML");

	// Конвертируем только то, что нужно
	Для Каждого СтрокаВерсия Из ТаблицаВерсий Цикл

		СтрокаВерсия.ТекстHTML = "";

		Если НЕ ПустаяСтрока(Версия) И СтрокаВерсия.Версия <> Версия Тогда
			Продолжить;
		КонецЕсли;

		СтрокаВерсия.ТекстHTML = КонвертироватьВHTML(СтрокаВерсия.Текст);

	КонецЦикла;

КонецПроцедуры

// Настройки парсера Markdown, единые для всего модуля
//
// Возвращаемое значение:
//	MarkdownНастройки - набор расширений GitHub Flavored Markdown
//
Функция НастройкиMarkdown()

	Если НастройкиMarkdownКэш = Неопределено Тогда
		НастройкиMarkdownКэш = Markdown.НастройкиGitHubLike();
	КонецЕсли;

	Возврат НастройкиMarkdownКэш;
