...
Процедура Конвертировать(Знач Параметры) Экспорт
	...
	Иначе
		Если мФормат = Перечисления.ФорматChangeLog.ПолныйHTMLВсе Тогда
			...
			Структура = Новый Структура();
			Структура.Вставить("Title", мВерсия);
			Структура.Вставить("Text", MarkdownВHTML(ТекстОсновногоФайла));
			Текст = СтроковыеОперации.ЗаменитьПараметры(Текст, Структура);
		Иначе
			Текст = MarkdownВHTML(ТекстОсновногоФайла);
		КонецЕсли;
	КонецЕсли;
...
Функция MarkdownВHTML(Знач ИсходнаяСтрока) Экспорт
	...
	ТекстHTML = ЗаменитьПоРегулярномуВыражению(ТекстHTML, "<h2>\[(.*)\]<\/h2>", "<h2>$1</h2>");
