...
Функция КонвертироватьФайлИлиКаталогВHTML(Знач КаталогMD, Знач КаталогHTML) Экспорт

	ИмяФайлаЗапуска = ИмяФайлаЗапускаDocsinum();

	Массив = Новый Массив();
	Массив.Добавить("parse");
	Массив.Добавить("--in " + ФайловыеОперации.ОбернутьПутьВКавычки(КаталогMD));
	Массив.Добавить("--out " + ФайловыеОперации.ОбернутьПутьВКавычки(КаталогHTML));
	Массив.Добавить("--template gfm");
	
	Команда = Новый КонтролируемыйПроцесс();	
	Команда.УстановитьКоманду(ФайловыеОперации.ОбернутьПутьВКавычки(ИмяФайлаЗапуска));
	...
	Если КодВозврата <> 0 Тогда
		ОбщегоНазначения.ЗавершениеРаботыОшибка("Ошибка запуска приложения docsinum. Код возврата <%1>", КодВозврата);
	КонецЕсли;
...
Функция ИмяФайлаЗапускаDocsinum() Экспорт

	ИмяФайлаЗапуска = ОбъединитьПути(ОбщегоНазначения.КаталогМакетов(), "docsinum");
	Если ОбщегоНазначения.ЭтоWindows() Тогда
		ИмяФайлаЗапуска = ИмяФайлаЗапуска + ".exe";
	КонецЕсли;

	Возврат ИмяФайлаЗапуска;

КонецФункции

Функция КонвертироватьВHTML(Знач ТекстMD) Экспорт

	ВременныйФайлMD = ВременныеФайлы.НовоеИмяФайла("md");
	ВременныйФайлHTML = ВременныеФайлы.НовоеИмяФайла("html");
	ФайловыеОперации.ЗаписатьТекстФайла(ВременныйФайлMD, ТекстMD);

	КодВозврата = КонвертироватьФайлИлиКаталогВHTML(ВременныйФайлMD, ВременныйФайлHTML);
	Результат = ФайловыеОперации.ПрочитатьТекстФайла(ВременныйФайлHTML);
	
	УдалитьФайлы(ВременныйФайлMD);
	УдалитьФайлы(ВременныйФайлHTML);
...
Процедура Конвертировать(Знач Параметры) Экспорт
	...
	Для Каждого СтрокаВерсия Из ТаблицаВерсий Цикл
		...
		Если СтрЗаканчиваетсяНа(Параметры.Шаблон, ".html") Тогда
			ТекстВерсии = СтрокаВерсия.ТекстHTML; //КонвертироватьВHTML(СтрокаВерсия.Текст);
...
Процедура ДополнитьТаблицуВерсийКолонкойТекстHTML(ТаблицаВерсий, Знач Версия);
	...
	ВременныйКаталогMD = ВременныеФайлы.СоздатьКаталог("md");
	...
	Для Каждого СтрокаВерсия Из ТаблицаВерсий Цикл
		...
		ВременныйФайлMD = ОбъединитьПути(ВременныйКаталогMD, СтрокаВерсия.Версия + ".md");
		ФайловыеОперации.ЗаписатьТекстФайла(ВременныйФайлMD, СтрокаВерсия.Текст);

	КонецЦикла;

	// Конвертируем все файлы md
	КонвертироватьФайлИлиКаталогВHTML(ВременныйКаталогMD, ВременныйКаталогMD);
	...
	Для Каждого СтрокаВерсия Из ТаблицаВерсий Цикл
		...
		ИмяФайлаHTML = ОбъединитьПути(ВременныйКаталогMD, СтрокаВерсия.Версия + ".html");
		СтрокаВерсия.ТекстHTML = ФайловыеОперации.ПрочитатьТекстФайла(ИмяФайлаHTML);

	КонецЦикла;
	УдалитьФайлы(ВременныйКаталогMD);	
