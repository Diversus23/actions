...
Функция КопироватьФайлы(Знач Источник, Знач Приемник, Знач ВключаяПодкаталоги = Истина,
	...
	Источник = УдалитьПоследнийРазделительПути(Источник);
	Приемник = УбратьКавычкиВокругПути(Приемник);
	Приемник = УдалитьПоследнийРазделительПути(Приемник);
	ОбеспечитьКаталог(Приемник);

	Если КаталогСуществует(Источник) Тогда
		ПутьИсточника = Источник;
		Файлы = НайтиФайлы(ПутьИсточника, "*", ВключаяПодкаталоги);
	Иначе
		Файл = Новый Файл(Источник);
		ПутьИсточника = Файл.Путь;
		Маска = Файл.ИмяФайла;
		Файлы = НайтиФайлы(ПутьИсточника, Маска, ВключаяПодкаталоги);
	КонецЕсли;
	
	Для Каждого Файл Из Файлы Цикл
		ПутьКФайлу = ОбъединитьПути(Приемник, Сред(Файл.Путь, СтрДлина(Источник) + 2));
		ПутьКопирования = ОбъединитьПути(ПутьКФайлу, Файл.Имя);
		Если Файл.ЭтоКаталог() Тогда
			ОбеспечитьКаталог(Файл.ПолноеИмя);
		Иначе
			ОбеспечитьКаталог(Файл.Путь);
			Сообщить(Файл.ПолноеИмя);
			Сообщить(ПутьКопирования);
			КопироватьФайл(Файл.ПолноеИмя, ПутьКопирования);
			//Лог.Информация("Файл %1 скопирован в %2", Файл.ПолноеИмя, ПутьКопирования);
		КонецЕсли;
	КонецЦикла;

	Если УдалитьПослеКопирования = Истина Тогда
		УдалитьФайлы(ПутьИсточника);
		Лог.Информация("Файл(ы) %1 удален(ы)", ПутьИсточника);
	КонецЕсли;
