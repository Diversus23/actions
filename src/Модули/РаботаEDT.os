...
Функция УстановленныеВерсииEDT() Экспорт
	...
	Для Каждого Путь Из МассивПутейУстановки Цикл
		Если ФайловыеОперации.КаталогСуществует(Путь) Тогда			
			Маска = "1c-edt-*";
			ШаблонВерсии = "1c-edt-(\d+\.\d+\.\d+).*x86_64";
			МассивКаталогов = НайтиФайлы(Путь, Маска, Ложь);
			Для Каждого Файл Из МассивКаталогов Цикл
				Массив = ОбщегоНазначения.НайтиПоРегулярномуВыражению(Файл.ПолноеИмя, ШаблонВерсии);
				ИмяФайлаEdtCli = ОбъединитьПути(Файл.ПолноеИмя, ИмяEdtCli);
				Если Массив.Количество() > 1 И ФайловыеОперации.ФайлСуществует(ИмяФайлаEdtCli) Тогда
					//Лог.Информация(Файл.ПолноеИмя);
					Структура = Новый Структура("Версия, Путь", Массив[1], Файл.ПолноеИмя);
