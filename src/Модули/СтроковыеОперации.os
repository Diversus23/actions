...
// Заменяет параметры вида {{Параметр}} в тексте
//
// Параметры:
//	Текст - Строка - текст
//	Структура - Структура - где ключ имя параметра, а значение строка, которую надо заменить.
//
// Возвращаемое значение:
//	Строка - строка после замены
//
Функция ЗаменитьПараметры(Знач Текст, Знач Структура) Экспорт
	Результат = Текст;
	
	Для Каждого КлючЗначение Из Структура Цикл
		СтрокаЗамены = СтрШаблон("{{%1}}", КлючЗначение.Ключ);
		Результат = СтрЗаменить(Результат, СтрокаЗамены, КлючЗначение.Значение);
	КонецЦикла;
...
//	СокращатьНепечатаемыеСимволы - Булево - Использовать СокрЛП для подстрок. По умолчанию Ложь.
//
// Возвращаемое значение:
...
//
//  Возвращаемое значение:
//      Массив - массив значений, элементы которого - отдельные слова.
//
// Пример:
...
Функция ПолучитьПараметрыИзСтроки(Знач СтрокаПараметров) Экспорт
	...
	Для Каждого СтрокаПараметра Из МассивПодстрок Цикл
		...
		Если Лев(ЗначениеПараметра, 1) = СимволДвойныеКавычки
			...
			Отступ = 2;
			ЗначениеПараметра = Сред(ЗначениеПараметра, Отступ, СтрДлина(ЗначениеПараметра) - Отступ);
...
//
Функция ТолькоЦифрыВСтроке(Знач СтрокаПроверки, Знач УчитыватьЛидирующиеНули = Истина,
		Знач УчитыватьПробелы = Истина) Экспорт
...
//  ДопустимыеСимволы - Строка - строка для проверки.
//
// Возвращаемое значение:
...
//
Функция ТолькоКириллицаВСтроке(Знач СтрокаПроверки, Знач УчитыватьРазделителиСлов = Истина,
	...
	Для а = 1 По СтрДлина(СтрокаПроверки) Цикл
		...
		Условие = ((КодСимвола < 1040) ИЛИ (КодСимвола > 1103))
			И (КодыДопустимыхСимволов.Найти(КодСимвола) = Неопределено)
			И НЕ (НЕ УчитыватьРазделителиСлов И ЭтоРазделительСлов(КодСимвола));
		Если Условие Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
//         - Ложь, если строка содержит иные символы.
//
Функция ТолькоЛатиницаВСтроке(Знач СтрокаПроверки, Знач УчитыватьРазделителиСлов = Истина,
		ДопустимыеСимволы = "") Экспорт
...
Функция ЭтоУникальныйИдентификатор(Знач Строка) Экспорт
	...
	Для Позиция = 1 По СтрДлина(Строка) Цикл
		...
		Условие = КодСимвола(Шаблон, Позиция) = 88 // X
			...
			ИЛИ КодСимвола(Шаблон, Позиция) = 45 И КодСимвола(Строка, Позиция) <> 45;
...
Функция ДополнитьСтроку(Знач Строка, Знач ДлинаСтроки, Знач Символ = "0", Знач Режим = "Слева") Экспорт
	...
	Если КоличествоСимволовНадоДобавить > 0 Тогда
		...
		Иначе
			...
			ВызватьИсключение СтрШаблон("Не известный Режим у функции ДополнитьСтроку: ""%1""", Режим);
...
// Параметры:
//  ИсходнаяСтрока - Строка - исходная строка, которую необходимо дополнить символами;
//  ЧтоДобавляем - Строка  - строка, которую добавляем слева;
//
// Возвращаемое значение:
//  Строка - строка, дополненная символами.
//
...
Функция УдалитьПовторяющиесяСимволы(Знач Строка, Знач Символ, Знач Режим = "Слева") Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон("Не известный Режим у функции УдалитьПовторяющиесяСимволы: ""%1""", Режим);
...
Функция ЗаменитьОдниСимволыДругими(ЗаменяемыеСимволы, Строка, СимволыЗамены) Экспорт
	...
	Для НомерСимвола = 1 По СтрДлина(ЗаменяемыеСимволы) Цикл
		Результат = СтрЗаменить(Результат,
				Сред(ЗаменяемыеСимволы, НомерСимвола, 1),
				Сред(СимволыЗамены, НомерСимвола, 1));
	КонецЦикла;
...
Функция ПреобразоватьЧислоВАрабскуюНотацию(РимскоеЧисло, ИспользоватьКириллицу = Истина) Экспорт
	...
	Для Сч = 1 По ЧислоСимволов Цикл
		Если Сред(РимскоеЧисло, Сч, 1) = c1000 Тогда
			АрабскоеЧисло = АрабскоеЧисло + Тысяча;
		ИначеЕсли Сред(РимскоеЧисло, Сч, 1) = c500 Тогда
			АрабскоеЧисло = АрабскоеЧисло + Пятьсот;
		ИначеЕсли Сред(РимскоеЧисло, Сч, 1) = c100 Тогда
			Если (Сч < ЧислоСимволов) И ((Сред(РимскоеЧисло, Сч + 1, 1) = c500)
					ИЛИ (Сред(РимскоеЧисло, Сч + 1, 1) = c1000)) Тогда
				АрабскоеЧисло = АрабскоеЧисло - Сто;
			Иначе
				АрабскоеЧисло = АрабскоеЧисло + Сто;
			КонецЕсли;
		ИначеЕсли Сред(РимскоеЧисло, Сч, 1) = c50 Тогда
			АрабскоеЧисло = АрабскоеЧисло + Пятьдесят;
		ИначеЕсли Сред(РимскоеЧисло, Сч, 1) = c10 Тогда
			Если (Сч < ЧислоСимволов) И ((Сред(РимскоеЧисло, Сч + 1, 1) = c50)
					ИЛИ (Сред(РимскоеЧисло, Сч + 1, 1) = c100)) Тогда
				АрабскоеЧисло = АрабскоеЧисло - Десять;
			Иначе
				АрабскоеЧисло = АрабскоеЧисло + Десять;
			КонецЕсли;
		ИначеЕсли Сред(РимскоеЧисло, Сч, 1) = c5 Тогда
			АрабскоеЧисло = АрабскоеЧисло + Пять;
		ИначеЕсли Сред(РимскоеЧисло, Сч, 1) = c1 Тогда
			Если (Сч < ЧислоСимволов) И ((Сред(РимскоеЧисло, Сч + 1, 1) = c5)
					ИЛИ (Сред(РимскоеЧисло, Сч + 1, 1) = c10)) Тогда
...
//
// Параметры:
//  Строка - Строка - текст, который надо преобразовать.
//
// Возвращаемое значение:
//  Строка - текст, с замененными символами.
//
...
//
// Параметры:
//  Строка - Строка - строка, с тегами.
//
// Возвращаемое значение:
//  ФорматированнаяСтрока - текст, в виде форматной строки.
//
// Пример:
...
//
// Возвращаемое значение:
//   Число - Полученное число или Неопределено, если нельзя преобразовать.
//
Функция СтрокаВЧисло(Знач ИсходнаяСтрока) Экспорт
	...
	СимволПробела = 32;
	СимволНеразрывногоПробела = 160;
	СимволЗакрывающаяСкобка = 41;
	СимволЗапятая = 44;
	СимволТочка = 46;
	СимволНоль = 47;
	СимволОдин = 48;
	...
	Для НомерСимвола = 1 По СтрДлина(ИсходнаяСтрока) Цикл
		...
		Если КодСимвола = СимволПробела ИЛИ КодСимвола = СимволНеразрывногоПробела Тогда
			Продолжить;
		...
		ИначеЕсли КодСимвола = СимволЗакрывающаяСкобка Тогда // Закрывающая скобка.
			Если НЕ ЗнакОтрицательный ИЛИ Результат = 0 Тогда // Не было открывающей скобки или нет числа.
				Возврат Неопределено;
			КонецЕсли;
			Продолжить;
		ИначеЕсли КодСимвола = СимволЗапятая ИЛИ КодСимвола = СимволТочка Тогда // Запятая или точка.
		...
		ИначеЕсли КодСимвола > СимволНоль И КодСимвола < СимволДевять Тогда // Число.
			Если ЗнаковПослеЗапятой <> -1 Тогда
				ЗнаковПослеЗапятой = ЗнаковПослеЗапятой + 1;
			КонецЕсли;
			Число = КодСимвола - СимволОдин;
			Результат = Результат * Десять + Число;
		Иначе
			Возврат Неопределено;
		КонецЕсли;
	КонецЦикла;
	...
	Если ЗнаковПослеЗапятой > 0 Тогда
		Результат = Результат / Pow(Десять, ЗнаковПослеЗапятой);
	КонецЕсли;
...
//  РимскаяЕдиница - Строка - символы, соответствующие римским цифрам I.
//	РимскаяПятерка - Строка - символы, соответствующие римским цифрам V.
//	РимскаяДесятка - Строка - символы, соответствующие римским цифрам X.
//
// Возвращаемое значение:
...
Функция ПреобразоватьЦифруВРимскуюНотацию(Цифра, РимскаяЕдиница, РимскаяПятерка, РимскаяДесятка)
	...
	Иначе
		ВызватьИсключение СтрШаблон("В функцию ПреобразоватьЦифруВРимскуюНотацию передана не цифра %1", Цифра);
	КонецЕсли;
...
//
// Параметры:
//	СтрокаПодстановки - Строка - исходная строка.
//  Параметр<n> - Строка - параметр для подстановки.
//
// Возвращаемое значение:
//	Строка - строка с вставленными параметрами.
//
...
Функция СравнитьВерсии(Знач ЭтаВерсия, Знач БольшеЧемВерсия) Экспорт
	...
	// Если будут подключены ассерты, то можно будет заменить
	Если ТипЗнч(ЭтаВерсия) <> Тип("Строка") Тогда
		ВызватьИсключение "СравнитьВерсии: Передали неверный параметр <ЭтаВерсия>. Ожидается строка, передано "
		+ ТипЗнч(ЭтаВерсия);
	КонецЕсли;
	Если ТипЗнч(БольшеЧемВерсия) <> Тип("Строка") Тогда
		ВызватьИсключение "СравнитьВерсии: Передали неверный параметр <БольшеЧемВерсия>. Ожидается строка, передано "
		+ ТипЗнч(ЭтаВерсия);
	КонецЕсли;
...
// Расскладывает строку с версией на компоненты
//
// Параметры:
//  пВерсия  - Строка - номер версии
//
// Возвращаемое значение:
//   Массив -Массив чисел
//
Функция МассивКомпонентВерсии(Знач пВерсия)
	...
	// Вырезаем все числа. Считаем, что любой разделитель является именно разделителем между компонентами,
	// а не дробным числом, например
...
// Минимизировать HTML и удаление всего лишнего из него.
//
// Параметры:
//  Текст  - Строка - текст HTML-документа
Функция МинимизироватьHTML(Знач Текст) Экспорт

	// Удалить комментарии
	Текст = ОбщегоНазначения.ЗаменитьПоРегулярномуВыражению(Текст,
		"<!--(.|\s)*?-->",
		"");
	// Удалить несколько последовательных пробелов
	Текст = ОбщегоНазначения.ЗаменитьПоРегулярномуВыражению(Текст,
		"(\s)+",
		"$1");
	// Удалить пробелы после тегов
	Текст = ОбщегоНазначения.ЗаменитьПоРегулярномуВыражению(Текст,
		"\>[^\S ]+",
		">");
	// Удалить пробелы до тегов
	Текст = ОбщегоНазначения.ЗаменитьПоРегулярномуВыражению(Текст,
		"[^\S ]+\<",
		"<");
	// Удалить пробелы после тегов
