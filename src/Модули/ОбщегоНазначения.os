...
//
Процедура ЗавершитьПриложение(Знач Идентификатор) Экспорт

	Если ТипЗнч(Идентификатор) = Тип("Число") Тогда
		Идентификатор = Формат(Идентификатор, "ЧГ=0");
	КонецЕсли;

	Если ЭтоWindows() Тогда
		СтрокаКоманды = "TASKKILL /T /PID " + Идентификатор;
	Иначе
		СтрокаКоманды = "pkill -9 " + Идентификатор;
	КонецЕсли;
	retCode = -1;
