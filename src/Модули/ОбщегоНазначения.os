...
Функция ВыполнитьПроверкуСистемы(Знач ВыполнятьПроверку = Истина) Экспорт
	...
	// 1С
	Успешно = ?(ВыполнятьПроверку, Работа1С.ПроверитьУстановку1C(), Ложь);
	...
	// sonar-scanner
	Успешно = ?(ВыполнятьПроверку, ОбщегоНазначения.НайтиПриложениеВСистеме("sonar-scanner"), Ложь);
	...
	Массив.Добавить("https://github.com/SonarSource/sonar-scanner-cli");
	ДобавитьПроверку(Таблица, Успешно, "sonar-scanner", Текст, Массив);	

	// aws
	Успешно = ?(ВыполнятьПроверку, ОбщегоНазначения.НайтиПриложениеВСистеме("aws"), Ложь);
	Текст = "Пайплайны содержащий копирование в S3 не смогут работать => Установите aws-cli";
	...
	// allure
	Успешно = ?(ВыполнятьПроверку, ОбщегоНазначения.НайтиПриложениеВСистеме("allure"), Ложь);
	...
	// allure
	Успешно = ?(ВыполнятьПроверку, ОбщегоНазначения.НайтиПриложениеВСистеме("Coverage41C"), Ложь);
