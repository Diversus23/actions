...
Функция ВесВерсииИзМассиваСтрок(РазрядыВерсииСтроками)
	...
	КоличествоЭлементовВСтарыхВерсиях = 4;
	КоэффициентИндекс1 = 1000000000;   
	КоэффициентИндекс2 = 1000000;
	КоэффициентИндекс3 = 1000;
	
	Результат = 
		Число(РазрядыВерсииСтроками[0]) * КоэффициентИндекс1
		+ Число(РазрядыВерсииСтроками[1]) * КоэффициентИндекс2
		+ Число(РазрядыВерсииСтроками[2]) * КоэффициентИндекс3;
		
	Если РазрядыВерсииСтроками.Количество() = КоличествоЭлементовВСтарыхВерсиях Тогда
		Результат = Результат + Число(РазрядыВерсииСтроками[3]); 
	КонецЕсли;
