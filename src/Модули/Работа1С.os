...
Функция ПрочитатьПараметры(Знач Команда, Знач МассивПараметров = Неопределено) Экспорт
	...
	Если МассивПараметров.Найти("user") <> Неопределено Тогда
		ПараметрыКоманды.Вставить(ПараметрПользователь, ЧтениеОпций.ЗначениеОпции("user"));
		Если ПараметрыКоманды.Пользователь = """""" 
			ИЛИ ПараметрыКоманды.Пользователь = "''"
			ИЛИ ПараметрыКоманды.Пользователь = "null" Тогда
			ПараметрыКоманды.Вставить(ПараметрПользователь, "");
		КонецЕсли;
	Иначе
		ПараметрыКоманды.Вставить(ПараметрПользователь, "");
	КонецЕсли;
	Если НЕ ПустаяСтрока(ПараметрыКоманды.Пользователь) И МассивПараметров.Найти("password") <> Неопределено Тогда
