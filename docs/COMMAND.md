...
Синонимы: [--path, -p].
- *prefix*: Префикс создаваемой выгрузки информационной базы. По умолчанию пусто (не обязательный).
Синонимы: [--prefix, -p].
- *format*: Формат сохранения выгрузки. По умолчанию: "yyyyMMddHHmmss" => "20230626175709" (обязательный).
Синонимы: [--format, -f].
