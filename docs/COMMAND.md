...
- [actions](#) Программная среда CI/CD для платформы 1С и не только. Позволяет выполнять сборку, доставку, тестирование ПО и многое из того, что может понадобиться в DevOps
  ...
  - [fs](#fs) Команды для работы с файловой системой
    ...
    - [newemptypath](#fs-newemptypath) Обеспечить пустой каталог. Если каталог не существует, то он будет создан.
Если каталог существует, то его содержимое будет очищено
    - [newpath](#fs-newpath) Обеспечить каталог. Если он не существует, то будет создан
...
- *path*: Локальный каталог, который необходимо обеспечить и при необходимости очистить (обязательный).
Синонимы: [--path, -p].
