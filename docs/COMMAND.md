...
- *processpath*: Каталог для сохранения информации о запущенных actions процессов (для закрытия командой process killpath).
...
- *pid*: PID процесса, который необходимо завершить (обязательный).
Синонимы: [--pid, -p].
