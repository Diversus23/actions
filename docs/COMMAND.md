...
| [`ftp-get`](#ftp-get) | Копирование файла с FTP-сервера |
| [`ftp-put`](#ftp-put) | Копирование файла на FTP-сервер |
...
- **-format** Формат файла с новостью по обновлению. Возможны следующие варианты:
   1. txt - текстовый формат.
   2. html - HTML-файл без оформления. Все, что внутри тегов <body>...</body>.
   3. htmlfull - готовый HTML-файл со всеми стилями и с полным оформлением.
  Если параметр не задан, то будет использован формат htmlfull (необязательный).
...
- **-login** Логин FTP-сервера (необязательный).
- **-password** Пароль FTP-сервера (необязательный).
- **-remote** Имя директории на FTP-сервере, что необходимо получить (обязательный).
Если не указано, то будет использован корневой путь "/".
- **-mask** Маска файлов для скачивания. Если не указана, то используется маска "*" (необязательный).
Пример: *.txt.
- **-path** Директория на локальном компьютере куда необходимо загрузить файлы (обязательный).
Пример:
* C:\Path\File.txt
* C:\Path\.
- **-recursive** Флаг рекурсивного копирования файлов с FTP-сервера (необязательный).
По умолчанию копирование выполняется только с указанной на сервере директории.
- **-port** Порт на FTP-сервере (необязательный). Если не указано, то будет использован 21 порт.
- **-timeout** Таймаут подключения к FTP-серверу (необязательный). По умолчанию - 30 секунд.
- **-settings** Путь к файлу настроек CI/CD в формате json. По умолчанию имя файла "env.json"  (необязательно).
- **-logfile** Вывод отладочных файлов в указанный лог-файл (необязательно).
...
- **-login** Логин FTP-сервера (необязательный).
- **-password** Пароль FTP-сервера (необязательный).
- **-local** Имя файла, маски или директория для отправки на сервер на локальном компьютере (обязательный).
Пример:
* C:\Path\File.txt
* C:\Path\*.txt
* C:\Path\.
