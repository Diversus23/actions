...
- [actions](#) Программная среда CI/CD для платформы 1С и не только. Позволяет выполнять сборку, доставку, тестирование ПО и многое из того, что может понадобиться в DevOps
  ...
  - [timing](#timing) Работа с замерами производительности выполнения команд actions
    - [emailable-report](#timing-emailable-report) Получение краткого отчета по замерам производительности actions состоящего из одного файла (возможно для отправки по почте или для сохранения в артефакты)
...
- *timingpath*: Профилирование работы команды (засекаем время и сохраняем в каталог).
Синонимы: [--timingpath, -t].
