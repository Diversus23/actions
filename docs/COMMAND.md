...
- [actions](#) Программная среда CI/CD для платформы 1С и не только. Позволяет выполнять сборку, доставку, тестирование ПО и многое из того, что может понадобиться в DevOps
  ...
  - [changelog](#changelog) Команды работы с ChangeLog.md
    ...
    - [check-version](#changelog-check-version) Проверить наличие версии в ChangeLog.md
    - [convert](#changelog-convert) Конвертировать ChangeLog.md в другой формат
    - [templates](#changelog-templates) Вывести список доступных шаблонов для оформления ChangeLog.md
  ...
  - [edt](#edt) Команды для работы с 1C:Enterprise Development Tools (EDT)
    ...
    - [versionepferf](#edt-versionepferf) Возвращает в консоль версию внешнего отчета или обработки из проекта EDT получаемую из исходного кода
    - [cli](#edt-cli) Интерфейс для работы с утилитой 1cedtcli, которая идет с дистрибутивом EDT взамен ring
      - [export](#edt-cli-export) Экспортирует проект 1C:EDT в .xml-файлы конфигурации (используя 1cedtcli)
    - [versions](#edt-versions) Список установленных версий EDT
  - [epferf](#epferf) Команды для работы с отчетами / обработками (EDT)
    - [distrib](#epferf-distrib) Создает дистрибутив отчета/обработки
    - [version](#epferf-version) Возвращает в консоль версию внешнего отчета или обработки из проекта EDT получаемую из исходного кода
  ...
  - [ibcmd](#ibcmd) Команды для работы с информационными базами 1С через утилиту ibcmd (без конфигуратора)
    - [build](#ibcmd-build) Сборка/обновление информационной базы 1С из проекта EDT (конфигурация или расширение)
    - [configapply](#ibcmd-configapply) Обновление конфигурации базы данных
    - [configcheck](#ibcmd-configcheck) Проверка конфигурации 1С
    - [configload](#ibcmd-configload) Загрузить конфигурацию из cf-файла и обновить конфигурацию базы данных
    - [configloadfromxml](#ibcmd-configloadfromxml) Загрузить конфигурацию из исходников XML (формат 1C) и обновить конфигурацию базы данных
    - [configsave](#ibcmd-configsave) Сохранить конфигурацию в cf-файл
    - [configsavetoxml](#ibcmd-configsavetoxml) Сохранить конфигурацию в исходники XML (формат 1C)
    - [create](#ibcmd-create) Создание информационной базы 1С: пустой или из шаблона (cf, dt, каталог XML)
    - [dump](#ibcmd-dump) Выгрузка информационной базы 1С в dt-файл
    - [extension](#ibcmd-extension) Работа с расширениями 1С
      - [delete](#ibcmd-extension-delete) Удалить расширение (или все расширения) из информационной базы 1С
      - [list](#ibcmd-extension-list) Список расширений информационной базы 1С
      - [load](#ibcmd-extension-load) Загрузить расширение из cfe-файла и обновить его в базе данных
      - [loadfromsrc](#ibcmd-extension-loadfromsrc) Загрузить расширение из исходников XML (формат 1C) и обновить его в базе данных
      - [properties](#ibcmd-extension-properties) Изменить свойства расширения (активность, безопасный режим и т.д.)
      - [save](#ibcmd-extension-save) Сохранить расширение в cfe-файл
      - [savetosrc](#ibcmd-extension-savetosrc) Сохранить расширение в исходники XML (формат 1C)
      - [update](#ibcmd-extension-update) Обновить расширение в базе данных
    - [restore](#ibcmd-restore) Загрузка информационной базы 1С из dt-файла
  - [infobase](#infobase) Команды для работы с информационными базами 1C
    ...
    - [runcode](#infobase-runcode) Выполнить произвольный код в режиме 1С:Предприятие
    - [setlegal](#infobase-setlegal) Подтверждение легальности получения обновлений (для конфигураций на базе БСП)
    - [updatecfg](#infobase-updatecfg) Обновление конфигурации, находящейся на поддержке
  ...
  - [markdown](#markdown) Команды для работы с произвольными файлами Markdown
    - [convert](#markdown-convert) Конвертировать произвольный файл Markdown в HTML по шаблону оформления
    - [templates](#markdown-templates) Вывести список доступных шаблонов для оформления Markdown
  ...
  - [netlenka](#netlenka) Работа с обфускацией (запутывание кода) сервиса https://netlenka.org
    ...
    - [onescript](#netlenka-onescript) Обфускация скрипта OneScript
...
- *file*: Имя файла ChangeLog.md в формате MarkDown, в котором необходимо проверить наличие версии.
Синонимы: [--file, -f].
...
Синонимы: [--out, -o].
- *template*: Шаблон для создания файла с новостью. По умолчанию "Simple.html". Доступные шаблоны: Github.html, Letter.html, Simple.html, SimpleText.txt, SimpleTextWithoutVersion.txt, SimpleWithoutVersion.html, Softonit.html, Standart.html (не обязательный).
- *version*: Из какой версии будет получено описание. Если не задано, то создастся для всего файла --in (необязательный).
- *maxversion*: Из файла --in будет получена максимальная версия и для нее построен файл --out (необязательный).
Синонимы: [--maxversion, --max].
- *all*: Выполняется формирование по всем версиям (необязательный).
- *minifyhtml*: Минимизировать HTML-файл. Имеет смысл только для HTML-файлов (необязательный).
...
- [x] Проверка установленного приложения **ibcmd**. *Проблема: Не будут работать команды ibcmd => Установите платформу 1С с компонентом ibcmd (8.3.20 и выше)*. Ссылки для установки: [https://releases.1c.ru/project/Platform83]
...
- *path*: Каталог с файлом модуля где указана версия (обязательный).
Синонимы: [--path, -p].
...
- *workspace*: Каталог рабочего пространства для запуска 1C:Enterprise Development Tools (необязательный).
Синонимы: [--workspace, -w].
- *edtversion*: Версия EDT для экспорта. Пример: "2021.1.5" Список установленных версий можно узнать набрав "oscript actions.os edt cli versions" (необязательный).
Синонимы: [--edtversion, -e].
- *processpath*: Каталог записи информация о процессе EDT. Если процесс зависнет, то система закроет его (не обязательный).
...
- *path*: Каталог где будет создан дистрибутив (обязательный).
Синонимы: [--path, -p].
- *files*: Список файлов, которые нужно включить в дистрибутив или маска файлов разделенные ";" (обязательный).
Синонимы: [--files, -f].
...
- *timeout*: Таймаут подключения к FTP-серверу. По умолчанию - 60 секунд (необязательный).
Синонимы: [--timeout, -t].
...
Команды для работы с информационными базами 1С через утилиту ibcmd (без конфигуратора).
...
- *dbpath*: Каталог файловой информационной базы 1С. Указывается либо --dbpath, либо --dbms (для серверной ИБ).
- *dbms*: Тип СУБД серверной ИБ: MSSQLServer, PostgreSQL, IBMDB2, OracleDatabase. Подключение напрямую к СУБД.
- *dbserver*: Сервер СУБД (обязательный для серверной ИБ).
- *dbname*: Имя базы данных на сервере СУБД (обязательный для серверной ИБ).
- *dbuser*: Пользователь сервера СУБД (необязательный).
- *dbpassword*: Пароль пользователя сервера СУБД (необязательный).
- *user*: Имя пользователя информационной базы (необязательный).
Синонимы: [--user, -u].
- *password*: Пароль пользователя информационной базы (необязательный).
Синонимы: [--password, --pwd].
- *v8version*: Версия (маска версии) платформы 1С, из которой берется ibcmd. Например: 8.3.27, 8.5. По умолчанию - последняя установленная (необязательный).
- *processpath*: Каталог записи информация о процессе. Если процесс зависнет, то система закроет его (не обязательный).
- *data*: Каталог данных ibcmd. По умолчанию создается временный каталог на время выполнения команды (необязательный).
- *dynamic*: Динамическое обновление конфигурации БД: auto, disable, force. По умолчанию - как в ibcmd (необязательный).
- *sessionterminate*: Завершение активных сеансов при необходимости монопольного доступа: disable, force (необязательный).
- *project*: Каталог, содержащий проект 1C:Enterprise Development Tools (обязательный).
Синонимы: [--project, -j].
- *extension*: Имя расширения. Если указано, то проект EDT собирается как расширение с этим именем (необязательный).
- *create*: Создать информационную базу с нуля из проекта EDT. Нельзя использовать вместе с --extension (необязательный).
- *path*: Каталог, куда сохранить промежуточную выгрузку проекта в XML. По умолчанию - временный каталог. **Важно!** Каталог будет очищен в процессе работы команды (необязательный).
Синонимы: [--path, -p].
- *workspace*: Каталог рабочего пространства для запуска 1C:Enterprise Development Tools (необязательный).
Синонимы: [--workspace, -w].
- *edtversion*: Версия EDT для экспорта. Пример: "2021.1.5". Список установленных версий можно узнать набрав "oscript actions.os edt versions" (необязательный).
Синонимы: [--edtversion, -e].
...
- *processpath*: Каталог записи информация о процессе. Если процесс зависнет, то система закроет его (не обязательный).
- *data*: Каталог данных ibcmd. По умолчанию создается временный каталог на время выполнения команды (необязательный).
- *noapply*: Не выполнять обновление конфигурации базы данных после загрузки (необязательный).
- *dynamic*: Динамическое обновление конфигурации БД: auto, disable, force. По умолчанию - как в ibcmd (необязательный).
- *sessionterminate*: Завершение активных сеансов при необходимости монопольного доступа: disable, force (необязательный).
- *file*: Путь к cf-файлу конфигурации (обязательный).
Синонимы: [--file, -f].

---

#### ibcmd configloadfromxml
...
- *dynamic*: Динамическое обновление конфигурации БД: auto, disable, force. По умолчанию - как в ibcmd (необязательный).
- *sessionterminate*: Завершение активных сеансов при необходимости монопольного доступа: disable, force (необязательный).
- *path*: Каталог с исходниками конфигурации в формате XML 1С (обязательный).
...
- *processpath*: Каталог записи информация о процессе. Если процесс зависнет, то система закроет его (не обязательный).
- *data*: Каталог данных ibcmd. По умолчанию создается временный каталог на время выполнения команды (необязательный).
- *file*: Путь к cf-файлу, куда сохранить конфигурацию (обязательный).
Синонимы: [--file, -f].
- *db*: Сохранить конфигурацию базы данных, а не основную конфигурацию (необязательный).

---

#### ibcmd configsavetoxml
...
- *processpath*: Каталог записи информация о процессе. Если процесс зависнет, то система закроет его (не обязательный).
- *data*: Каталог данных ibcmd. По умолчанию создается временный каталог на время выполнения команды (необязательный).
- *path*: Каталог, куда сохранить исходники конфигурации в формате XML 1С. **Важно!** Каталог будет очищен в процессе работы команды (обязательный).
Синонимы: [--path, --to, -p].
...
- *data*: Каталог данных ibcmd. По умолчанию создается временный каталог на время выполнения команды (необязательный).
- *noapply*: Не выполнять обновление конфигурации базы данных после загрузки (необязательный).
- *template*: Шаблон для создания ИБ: каталог с исходниками XML, cf-файл или dt-файл. Если не указан, будет создана пустая ИБ (необязательный).
Синонимы: [--template, -t].
- *locale*: Имя локали информационной базы, например ru_RU (необязательный).
- *sqloffs*: Смещение дат для Microsoft SQL Server: 0 или 2000 (необязательный).
...
- *processpath*: Каталог записи информация о процессе. Если процесс зависнет, то система закроет его (не обязательный).
- *data*: Каталог данных ibcmd. По умолчанию создается временный каталог на время выполнения команды (необязательный).
- *file*: Путь к dt-файлу, куда выгрузить информационную базу (обязательный).
Синонимы: [--file, -f].
...
- *processpath*: Каталог записи информация о процессе. Если процесс зависнет, то система закроет его (не обязательный).
- *data*: Каталог данных ibcmd. По умолчанию создается временный каталог на время выполнения команды (необязательный).
- *name*: Имя удаляемого расширения. Указывается либо --name, либо --all.
Синонимы: [--name, --extension, -n].
- *all*: Удалить все расширения. Указывается либо --name, либо --all.
...
- *dynamic*: Динамическое обновление конфигурации БД: auto, disable, force. По умолчанию - как в ibcmd (необязательный).
- *sessionterminate*: Завершение активных сеансов при необходимости монопольного доступа: disable, force (необязательный).
- *file*: Путь к cfe-файлу расширения (обязательный).
Синонимы: [--file, -f].
- *name*: Имя расширения. Если расширения нет в ИБ, оно будет создано (обязательный).
Синонимы: [--name, --extension, -n].

---

##### ibcmd extension loadfromsrc
...
- *dynamic*: Динамическое обновление конфигурации БД: auto, disable, force. По умолчанию - как в ibcmd (необязательный).
- *sessionterminate*: Завершение активных сеансов при необходимости монопольного доступа: disable, force (необязательный).
- *path*: Каталог с исходниками расширения в формате XML 1С (обязательный).
Синонимы: [--path, --from, -p].
- *name*: Имя расширения. Если расширения нет в ИБ, оно будет создано (обязательный).
Синонимы: [--name, --extension, -n].
...
Синонимы: [--name, --extension, -n].
- *active*: Активность расширения: yes, no (необязательный).
- *safemode*: Безопасный режим: yes, no (необязательный).
- *securityprofile*: Имя профиля безопасности (необязательный).
- *unsafeprotection*: Защита от опасных действий: yes, no (необязательный).
- *usedindistributedib*: Используется в распределенной информационной базе: yes, no (необязательный).
- *scope*: Область действия расширения: infobase, data-separation (необязательный).

---

##### ibcmd extension save
...
- *name*: Имя расширения (обязательный).
Синонимы: [--name, --extension, -n].
- *file*: Путь к cfe-файлу, куда сохранить расширение (обязательный).
Синонимы: [--file, -f].

---

##### ibcmd extension savetosrc
...
- *name*: Имя расширения (обязательный).
Синонимы: [--name, --extension, -n].
- *path*: Каталог, куда сохранить исходники расширения в формате XML 1С. **Важно!** Каталог будет очищен в процессе работы команды (обязательный).
Синонимы: [--path, --to, -p].
...
- *processpath*: Каталог записи информация о процессе. Если процесс зависнет, то система закроет его (не обязательный).
- *data*: Каталог данных ibcmd. По умолчанию создается временный каталог на время выполнения команды (необязательный).
- *file*: Путь к dt-файлу, из которого загрузить информационную базу (обязательный).
Синонимы: [--file, -f].
- *createdb*: Создать базу данных в случае отсутствия (необязательный).
...
Синонимы: [--txt, -t].
- *letter*: Путь к HTML-письму для рассылки, куда будет сохранена информация об обновлении. Необходимо заполнение одной из опций "--html" или "--txt" или "--letter" (не обязательный).
Синонимы: [--letter, -l].
...
Синонимы: [--uccode, --uc].
- *processpath*: Каталог записи информация о процессе. Если процесс зависнет, то система закроет его (не обязательный).
- *file*: Путь к файлу с кодом, который надо выполнить (обязательный).
Синонимы: [--file, -f].
- *code*: Произвольный код, который надо выполнить (не обязательный).
Синонимы: [--code, --text, --txt].
...
Синонимы: [--uccode, --uc].
- *processpath*: Каталог записи информация о процессе. Если процесс зависнет, то система закроет его (не обязательный).
- *file*: Путь к cf или cfe-файлу для обновления конфигурации (обязательный).
Синонимы: [--file, -f].
...
Команды для работы с произвольными файлами Markdown.
...
- *in*: Исходный файл в формате MarkDown, который необходимо сконвертировать.
Синонимы: [--in, -i].
- *out*: Файл результата в формате HTML.
Синонимы: [--out, -o].
- *template*: Шаблон оформления результата. По умолчанию "Softonit.html". Доступные шаблоны: Github.html, Letter.html, Simple.html, SimpleWithoutVersion.html, Softonit.html, Standart.html (не обязательный).
- *title*: Заголовок документа. Если не задан, используется значение опции --productname (необязательный).
- *productname*: Имя продукта (необязательный).
- *minifyhtml*: Минимизировать HTML-файл (необязательный).
- *maximgwidth*: Максимальная ширина изображения. Полезна для документов, которые будут отправлены по почте в формате HTML. Всем тегам img будет насильно задана указанная ширина (необязательный).
- *lazyload*: Использовать ленивую загрузку изображений. Полезна для документов в формате HTML, где много картинок и которые из-за этого будут долго открываться. Всем тегам img будет насильно задан class="lazyload" и в head HTML будет встроен скрипт lazysizes.js (необязательный).
...
- *path*: Каталог где хранятся json-отчеты по замерам производительности (обязательный).
Синонимы: [--path, -p].
- *file*: Имя html-файла куда сохранить отчет по замерам производительности по всем замерам в каталоге (обязательный).
Синонимы: [--file, -f].
...
- *pid*: PID процесса, который необходимо завершить (не обязательный).
Синонимы: [--pid, -p].
- *windowtitle*: Регулярное выражение заголовка окна для завершения. Пример: "Бухгалтерия" (не обязательный).
Синонимы: [--windowtitle, -t].
- *processname*: Заголовок окна или его маска, который необходимо завершить. Пример "1cv8" (не обязательный).
Синонимы: [--processname, -p].
...
- *processpath*: Каталог с json-файлам с информацией о запущенных процессах для закрытия (обязательный).
Синонимы: [--processpath, -p].
...
- *cmd*: Командная строка запускаемого процесса. Пример "notepad.exe" или "C:\Folder\File.exe" (обязательный).
Синонимы: [--cmd, -c].
- *arg*: Аргументы запускаемого процесса (не обязательный).
Синонимы: [--arg, -a].
- *wait*: Ожидать завершения. По умолчанию не ожидает (не обязательный).
Синонимы: [--wait, -w].
- *processpath*: Каталог записи информация процессе EDT. Если процесс зависнет, то система закроет его (не обязательный).
Синонимы: [--processpath, -p].
